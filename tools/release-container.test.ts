...
describe('container release registry controls', () => {
  ...
  it.each([
    ['stage:image: no signatures associated', 'found no attestations'],
    ['no signatures found', 'no attestations found'],
    ['no matching signatures', 'found no attestations']
  ])('treats cosign wording %j / %j as nothing left to clean', (signatureMessage, attestationMessage) => {
    // cosign has reworded these across releases; a missed alias turns a
    // successful clean into a hard failure that fails an entire release.
    const run = vi.fn((tool: string, args: string[]) => {
      if (tool === 'cosign' && args[0] === 'clean') return '';
      if (tool === 'cosign' && args[0] === 'download') {
        throw missing(args[1] === 'signature' ? signatureMessage : attestationMessage);
      }
      if (args[0] === 'delete') return '';
      throw missing();
    });

    expect(() => cleanupStagingImage({ reference: 'stage:image', run })).not.toThrow();
  });
  ...
  it('removes private staging references, tolerates absence, and propagates other failures', () => {
    ...
    const registryRefusal = Object.assign(new Error('403'), {
      stderr: 'Error: 403 Forbidden: requested access to the resource is denied'
    });
    const unreachable = Object.assign(new Error('connection reset'), { stderr: 'connection reset' });
    expect(() =>
      cleanupStagingImage({
    ...
    // cosign clean denial is the same token-scope class: GITHUB_TOKEN cannot
    // delete GHCR signature manifests. Warn, skip the post-clean verification
    // (the artifacts it would check are exactly those the token may not
    // remove), and continue to the tag delete.
    expect(() =>
      cleanupStagingImage({
        reference: 'stage:image',
        run: (tool: string, args: string[]) => {
          if (tool === 'cosign' && args[0] === 'clean') throw registryRefusal;
          if (tool === 'cosign' && args[0] === 'download') {
            throw new Error('download must be skipped after a refused clean');
          }
          if (args[0] === 'delete') throw registryRefusal;
          throw missing();
        }
      })
    ).not.toThrow();
    expect(() =>
      cleanupStagingImage({
        ...
        run: (tool: string, args: string[]) => {
          if (tool === 'cosign' && args[0] === 'clean') throw registryRefusal;
          if (args[0] === 'delete') return '';
          if (args[0] === 'inspect') throw missing('name unknown');
          throw missing();
        }
      })
    ).not.toThrow();
    // A bare "denied" without the OCI denial shape is NOT the token-scope
    // refusal and must still propagate.
    expect(() =>
      cleanupStagingImage({
        reference: 'stage:image',
        run: (tool: string, args: string[]) => {
          if (tool === 'cosign' && args[0] === 'clean') throw denied;
          throw missing();
        }
      })
    ).toThrow(denied);
    // GHCR answers manifest DELETEs with UNSUPPORTED for any identity —
    // cosign clean can never succeed there. Same tolerated class.
    const unsupported = Object.assign(new Error('UNSUPPORTED'), {
      stderr: 'DELETE https://ghcr.io/v2/x/manifests/sha256:abc: UNSUPPORTED: The operation is unsupported.'
    });
    expect(() =>
      cleanupStagingImage({
        reference: 'stage:image',
        run: (tool: string, args: string[]) => {
          if (tool === 'cosign' && args[0] === 'clean') throw unsupported;
          if (tool === 'cosign' && args[0] === 'download') {
            throw new Error('download must be skipped after an unsupported clean');
          }
          if (args[0] === 'delete') throw unsupported;
          throw missing();
        }
      })
    ).not.toThrow();
