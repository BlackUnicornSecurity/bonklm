...
describe('listSourceFiles', () => {
  ...
  it('skips a directory it cannot read and warns with the offending path (fail-open seam made observable)', () => {
    ...
    const warn = vi.fn();
    expect(listSourceFiles('/root', { readdir: readdir as never, warn })).toEqual([]);
    expect(warn).toHaveBeenCalledWith(expect.stringMatching(/skipped unreadable directory '?locked/));
  });
...
describe('CLI (real child process)', () => {
  it('exits 0 and reports the all-Apache tree clean against the real repo', () => {
    const output = execFileSync('node', [SCRIPT], { cwd: REPO_ROOT, encoding: 'utf-8' });
    expect(output).toMatch(/no OSS→EE boundary violations/);
  });

  it('exits 1 with a diagnostic when an OSS package imports an EE package', () => {
    ...
    expect(stderr).toMatch(/license-boundary check failed/);
    expect(stderr).toContain('@x/ee');
  });
});
