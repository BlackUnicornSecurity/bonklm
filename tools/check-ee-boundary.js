 ...
 *
 *   A. License classifier — every workspace package under `packages/` and every
 *      explicitly publishable Tier-B package under `tools/` must declare a
 *      RECOGNIZED license:
 ...
 * gate decides whether a tool is internal Tier A or publishable Tier B. This gate
 * then treats Tier-B packages as public OSS release roots and enforces that they
 * cannot name EE code. License tiers here are ALWAYS called "OSS/EE" (or
 * "Apache/BSL"), never "Tier A/B", to avoid the vocabulary collision.
 *
 * DESIGN NOTES
 * ------------
 ...
 * - Private Tier-A `tools/*` packages remain out of scope. Tier-B tools are
 *   publishable public artifacts and are classified and scanned like OSS roots.
 *
...
export const TIER_B_LICENSES = new Set(['Apache-2.0', 'MIT']);
...
function addTierBPackages(index, toolsDir) {
  if (!existsSync(toolsDir)) return;
  for (const entry of readdirSync(toolsDir, { withFileTypes: true })) {
    if (!entry.isDirectory()) continue;
    const absDir = join(toolsDir, entry.name);
    const pkg = readJson(join(absDir, 'package.json'));
    if (pkg?.workspacePolicy !== 'tier-b-publishable') continue;
    if (typeof pkg.name !== 'string' || pkg.name.length === 0) {
      index.errors.push({
        dir: `../tools/${entry.name}`,
        name: null,
        license: licenseOf(pkg),
        reason: 'manifest has no "name"'
      });
      continue;
    }
    const license = licenseOf(pkg);
    if (!TIER_B_LICENSES.has(license)) {
      index.errors.push({
        dir: `../tools/${entry.name}`,
        name: pkg.name,
        license,
        reason: 'Tier-B package requires a recognized permissive license'
      });
      continue;
    }
    const record = {
      ...
      manifestPath: `tools/${entry.name}/package.json`
    };
    index.list.push(record);
    index.byName.set(pkg.name, record);
  }
}
 ...
 * absolute paths. Any unreadable directory fails closed because skipped source
 * cannot be proven free of EE imports.
 */
export function listSourceFiles(dirAbs, { readdir = readdirSync } = {}) {
  const out = [];
  const stack = [dirAbs];
  while (stack.length > 0) {
    ...
    try {
      ...
      throw new Error(`check-ee-boundary: unreadable directory '${relative(dirAbs, cur)}'`, { cause: error });
    }
 ...
 */
export function checkEeBoundary({ packagesDir, toolsDir, readSource, listFiles } = {}) {
  const dir = packagesDir ?? PACKAGES_DIR;
  const toolDir = toolsDir ?? (packagesDir === undefined ? TOOLS_DIR : join(dirname(dir), 'tools'));
  const read = readSource ?? (p => readFileSync(p, 'utf-8'));
  const list = listFiles ?? listSourceFiles;

  const index = buildPackageIndex(dir);
  addTierBPackages(index, toolDir);
  ...
  if (result.violations.length > 0) {
    ...
    for (const v of result.violations) {
      if (v.kind === 'dynamic-nonliteral') {
        ...
        lines.push(
          `    - ${v.fromPath ?? `packages/${v.fromDir}/package.json`}: OSS package ${v.from} declares EE package ${v.target} as a dependency — \`${v.spec}\``
        );
      } else {
        lines.push(
