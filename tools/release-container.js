...
function missingCosignArtifact(error) {
  const detail = `${error?.stderr ?? ''}\n${error?.message ?? ''}`;
  // cosign words "nothing to download" several ways and has changed them
  // across releases: current builds say "no signatures associated" and
  // "found no attestations", older ones "no signatures found" / "no
  // attestations found". Missing an alias turns a successful clean into a
  // hard failure, so match the noun rather than the phrasing.
  return (
    missingManifest(error) ||
    /no (?:matching )?signatures\b|no attestations\b|found no (?:signatures|attestations)\b/i.test(detail)
  );
}
...
// GITHUB_TOKEN has no packages:delete, and GHCR's OCI endpoint answers
// manifest DELETEs with UNSUPPORTED regardless of scope — cosign clean can
// never succeed there for the Actions identity. Both are token/registry-scope
// limitations, not release defects — the same class the skopeo delete below
// already tolerates. Match the OCI distribution-spec denial shape or GHCR's
// explicit UNSUPPORTED rather than a bare word, so a proxy block page or an
// unrelated "denied" string cannot masquerade as the registry refusal.
function registryDenied(error) {
  const detail = `${error?.stderr ?? ''}\n${error?.message ?? ''}`;
  return (
    /requested access to the resource is denied|denied:|UNSUPPORTED: The operation is unsupported/i.test(detail) ||
    (/\b403\b/.test(detail) && /forbidden|denied/i.test(detail))
  );
}

// cosign stderr is registry-influenceable; strip control characters before it
// reaches the CI log (ADR-0001).
...
export function cleanupStagingImage({ reference, run, retries = 3, pause = waitFor }) {
  let cleanDenied = false;
  try {
    run('cosign', ['clean', '--force', reference], {});
  } catch (error) {
    if (registryDenied(error)) {
      console.error(`release-container: warning: cosign clean refused for ${reference} (${logSafeError(error)})`);
      cleanDenied = true;
    } else if (!missingCosignArtifact(error)) {
      throw error;
    }
  }
  // A refused clean leaves the artifacts it was not allowed to remove, and a
  // partial clean cannot be distinguished from one, so verification cannot run
  // fail-safely in that state and is skipped. Only a successful (or vacuous)
  // clean is verified.
  if (!cleanDenied) {
