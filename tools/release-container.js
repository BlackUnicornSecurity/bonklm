...
// GITHUB_TOKEN has no packages:delete, so GHCR refuses cosign clean's
// signature-manifest deletion for the Actions identity. That is a token-scope
// limitation, not a release defect — the same class the skopeo delete below
// already tolerates. Match the OCI distribution-spec denial shape rather than
// a bare word, so a proxy block page or an unrelated "denied" string cannot
// masquerade as the token-scope refusal.
function registryDenied(error) {
  const detail = `${error?.stderr ?? ''}\n${error?.message ?? ''}`;
  return (
    /requested access to the resource is denied|denied:/i.test(detail) ||
