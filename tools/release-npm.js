...
function assertRegistryPackage(run, pkg, requireProvenance) {
  ...
  if (foundIntegrity !== pkg.integrity) throw new Error(`Registry integrity mismatch: ${spec}`);
...
function channelState(packages, channel, run) {
  ...
  if (current.some(version => version !== null && typeof version !== 'string')) {
    throw new Error(`Invalid existing ${channel} dist-tag`);
  }
  const present = current.filter(version => typeof version === 'string');
  if (present.some(version => !isValidSemver(version))) throw new Error(`Invalid existing ${channel} dist-tag`);
  const versions = [...present, packages[0].version];
  if (channel === 'latest' && versions.some(version => parseSemver(version).prerelease !== null)) {
    throw new Error('latest channel accepts stable versions only');
  }
  if (channel === 'next' && versions.some(version => parseSemver(version).prerelease === null)) {
    throw new Error('next channel accepts prerelease versions only');
  }
  const newer = present.filter(version => compareSemver(version, packages[0].version) > 0);
  const target = present.filter(version => version === packages[0].version);
  if (newer.length === packages.length && new Set(newer).size === 1) return { state: 'newer', version: newer[0] };
  if (target.length === packages.length) return { state: 'target', version: packages[0].version };
  if (newer.length > 0) throw new Error(`Refusing a mixed or rollback ${channel} promotion`);
  return { state: 'promote', version: packages[0].version };
}
...
function promoteGroup(group, channel, run, originalEntries) {
  if (channelState(group, channel, run).state === 'newer') return;
  const original =
    originalEntries ??
...
function effectivePackages(manifest, channel, run) {
  ...
  return groups.flatMap(group => {
    const state = channelState(group, channel, run);
    if (!['target', 'newer'].includes(state.state)) throw new Error(`Incomplete ${channel} dist-tag promotion`);
...
export function promoteBundle({ dir, channel, snapshotPath, run, trusted }) {
  ...
  // Registry dist-tag reads lag the writes just performed (observed:
  // all adds succeed, the immediate read-back still shows the old
  // channel state). Retry the final channel verification with backoff.
  const attempts = 12;
  for (let attempt = 1; ; attempt += 1) {
    try {
      ...
      if (attempt >= attempts) throw error;
      console.error(
        `release-npm: channel ${channel} not consistent yet (attempt ${attempt}/${attempts}) — retrying in 45s`
      );
      Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 45_000);
    }
  }
