...
// Registry-lag retries must not sleep against a fake registry. Set here, not
// per-suite: Atomics.wait blocks the worker thread, so vitest's testTimeout
// cannot interrupt a suite that reaches a retry without this. A raw
// assignment (not vi.stubEnv) survives the afterEach vi.unstubAllEnvs below.
// Mirror the production parser rather than testing for nullish: a set-but-
// empty value (a CI expression that resolved to nothing) is rejected there
// and would restore the full delay here.
if (!/^\d+$/.test(process.env.BONKLM_RELEASE_RETRY_DELAY_MS ?? '')) {
  process.env.BONKLM_RELEASE_RETRY_DELAY_MS = '0';
}
