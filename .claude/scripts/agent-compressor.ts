...
function parseAgentFile(filePath: string): { metadata: AgentMetadata; persona: PersonaElements; rawContent: string } | null {
 ...
 */
function compressIdentity(identity: string, role: string): string {
 ...
 */
function compressVoice(style: string): string {
  if (!style) return '';
 ...
 */
function compressPrinciples(principles: string): string {
 ...
 */
function compressAgent(metadata: AgentMetadata, persona: PersonaElements): CompressedPersona {
  const extended: CompressedPersona['extended'] = {};

  if (persona.identity) extended.identity_full = persona.identity;
  if (persona.principles) extended.principles_full = persona.principles;
  if (persona.communicationStyle) extended.communication_style_full = persona.communicationStyle;
  if (persona.inherentBiases) extended.inherent_biases = persona.inherentBiases;
 ...
 */
function generateCompactMarkdown(compressed: CompressedPersona): string {
...
function processAgent(filePath: string, dryRun: boolean = false): CompressionResult | null {
...
function printStats(results: CompressionResult[]) {
  console.log('\n=== COMPRESSION STATISTICS ===\n');

  // Group by module
  const byModule: Record<string, CompressionResult[]> = {};
