 ...
 */
function getDomainTag(module: string): string {
  return TAG_TAXONOMY.domain[module as keyof typeof TAG_TAXONOMY.domain] || 'software';
}

/**
 * Extract function tags from text
 */
function extractFunctionTags(text: string): string[] {
 ...
 */
function extractSpecialtyTags(text: string): string[] {
  const lowerText = text.toLowerCase();
  const tags = new Set<string>();
 ...
 */
function generateAgentTags(agent: AgentEntry): string {
  const tags: string[] = [];
 ...
 */
function generateWorkflowTags(workflow: WorkflowEntry): string {
  const tags: string[] = [];
 ...
 */
function generateAgentSummary(agent: AgentEntry): string {
  ...
  const hasActionVerb = actionVerbs.some(v =>
    firstWord.includes(v) || summary.toLowerCase().startsWith(v)
  );
 ...
 */
function generateWorkflowSummary(workflow: WorkflowEntry): string {
 ...
 */
function compressAgent(agent: AgentEntry): MicroEntry {
  return {
    name: agent.name,
 ...
 */
function compressWorkflow(workflow: WorkflowEntry): MicroEntry {
  return {
    name: workflow.name,
 ...
 */
function parseAgentManifest(content: string): AgentEntry[] {
 ...
 */
function parseWorkflowManifest(content: string): WorkflowEntry[] {
  const rows = parseCSV(content);
  if (rows.length < 2) return [];
...
function validateCompleteness(
  fullEntries: Array<{ name: string; module: string }>,
  microEntries: Array<{ name: string; module: string }>
): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];

  // Create unique keys combining name and module
  const makeKey = (e: { name: string; module: string }) => `${e.module}:${e.name}`;
 ...
 */
function validateMicroEntry(entry: MicroEntry, index: number): string[] {
  const errors: string[] = [];
...
async function main() {
  ...
  const microWorkflows: MicroEntry[] = [];
  ...
  const agentValidation = validateCompleteness(
    agents.map(a => ({ name: a.name, module: a.module })),
    microAgents.map(a => ({ name: a.name, module: a.module }))
  );

  const workflowValidation = validateCompleteness(
    workflows.map(w => ({ name: w.name, module: w.module })),
    microWorkflows.map(w => ({ name: w.name, module: w.module }))
  );
  ...
  const agentStats: CompressionStats = {
  ...
  const workflowStats: CompressionStats = {
