...
import { loadManifest, normalizeLicense, selectPublishableRoots, shippedClosure } from './lib-shipped-closure.mjs';
...
export function classifyLicense(expr) {
  ...
  if (hasOr) {
    return e.split(/\s+OR\s+/i).some(p => classifyLicense(p.trim()) === 'ok')
      ? 'ok'
      : /LGPL/i.test(e)
        ? 'lgpl'
        : 'flagged';
  ...
  if (hasAnd) {
    return e.split(/\s+AND\s+/i).every(p => classifyLicense(p.trim()) === 'ok') ? 'ok' : 'flagged';
  }
...
export function fullProdLicenseMap({ repoRoot, run }) {
  const map = new Map();
  try {
...
export function main(options) {
  const {
  ...
  } = Object.assign(
    {
      ...
      exit: process.exit,
      licenseMapLoader: fullProdLicenseMap,
      log: console.log,
      repoRoot: REPO_ROOT,
      write: writeFileSync
    },
  ...
  const args = parseArgs(argv, { error: log, exit });
  if (!args) return null;
  if (args.help) {
    log('Usage: node scripts/license-audit.mjs [--json out.json] [--full]');
    exit(0);
    return null;
  }

  const roots = suppliedRoots ?? selectPublishableRoots(repoRoot, args.roots);
  const closure = suppliedClosure ?? shippedClosure({ repoRoot, roots });
  const byLicense = {};
  ...
  log('BonkLM license gate — SHIPPED production closure');
  log(`shipped third-party packages: ${[...closure.values()].filter(i => !i.viaWorkspace).length}`);
  log('license distribution:');
  for (const lic of Object.keys(byLicense).sort()) {
    log(`  ${String(byLicense[lic].length).padStart(3)}  ${lic}`);
  }
  log('');
  ...
  if (args.full || args.json) {
    for (const [key, lic] of peerLicenseMap ?? licenseMapLoader({ repoRoot, run: execFileSync })) {
      if (shippedKeys.has(key)) continue;
      if (classifyLicense(lic) !== 'ok') peerFlags.push({ key, license: lic });
    }
  }
  if (args.full && peerFlags.length) {
    log(`CONSUMER-AWARENESS — non-permissive licenses outside the default install closure: ${peerFlags.length}`);
  ...
  if (flagged.length) {
    log(`RESULT: FAIL — ${flagged.length} non-permissive license(s) in BonkLM's shipped closure:`);
    for (const f of flagged) log(`  ✗ ${f.name}@${f.version} — ${f.license} (${f.verdict})`);
    exit(1);
    return { byLicense, closure, flagged, peerFlags };
  }
  log('RESULT: PASS — BonkLM ships only permissive-licensed dependencies.');
  exit(0);
  return { byLicense, closure, flagged, peerFlags };
}
