...
#
# Packs every publishable package (npm pack — honouring each package's `files`
# allow-list) and runs gitleaks over the EXTRACTED tarball contents: the actual
# bytes a consumer downloads from npm. Zero secrets must ship. Findings are
...
mkdir -p "$PACKED" "$EXTRACT"

# Publishable (non-private) package directories.
DIRS="$(node -e '
const fs = require("fs");
for (const d of fs.readdirSync("packages")) {
  ...
  if (!p.private) process.stdout.write("packages/" + d + "\n");
}')"
