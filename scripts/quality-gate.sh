...
# "Canonical project version"). The evidence tree is keyed by the BASE version
# (prerelease suffix stripped). Override the location with BONKLM_QA_EVIDENCE_DIR.
VERSION="$(node -p "require('./packages/core/package.json').version" 2>/dev/null || echo "unknown")"
BASE_VERSION="${VERSION%%-*}"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
EVIDENCE_DIR="${BONKLM_QA_EVIDENCE_DIR:-.quality-gate-evidence/${BASE_VERSION}}"
...
if [ "$FAST" -eq 0 ]; then
  ...
  BUILD_STATUS="${STATUSES[${#STATUSES[@]} - 1]}"
  # tsc never cleans dist/: a renamed/deleted src file leaves orphaned emit
  # that rides the tarball (files: ["dist"]). Assert every built dist/ file
  # has a src/ twin BEFORE downstream gates evaluate the artifacts, so a
  # stale tree cannot produce false-green tarball/type-surface results.
  run_gate "stale-dist" node tools/check-stale-dist.js
...
run_gate "gitleaks-ignore-integrity" node tools/check-gitleaks-ignore.js
...
# Changesets `linked` aligns versions among packages already present in a plan,
# but does not add untouched members. Fail if an active BonkLM family release
# omits any of the 52 publishable packages or targets more than one version.
run_gate "release-plan" node tools/check-release-plan.js
run_gate "version-pin" bash scripts/check-version-pin.sh
...
# ---- Edge node:* allowlist gate ------------------------------------------
# Statically walk the import graph of the edge subpath entry
# (packages/core/src/edge/index.ts) and fail if the set of node:* built-ins it
# reaches diverges from the allowlist documented in architecture.md §6 — so a
# future edge-exported factory cannot silently drag a new Node built-in into the
# edge graph (breaking Deno/Bun/workerd portability). Dependency-free + instant
# (reads source text only), so it runs in fast mode too, mirroring the CI
# edge-node-builtins job.
run_gate "edge-node-builtins" node tools/check-edge-node-builtins.js
...
run_gate "diff-coverage" pnpm run check:diff-coverage
...
if [ "$FAST" -eq 0 ]; then
  ...
  # Project security-regression suite: deterministic replay of the committed,
  # hash-pinned, labelled corpora through every content-surface detection
  # component, failing on ANY per-component recall/FPR drop vs the committed
  # baseline (packages/core/benchmarks/security-regression/baseline.json).
  # Default lives in-repo so CI needs no env setup; BONKLM_SEC_REGRESSION
  # overrides the path for project-specific harnesses (.mjs/.js run under
  # node, anything else under bash).
  SEC_REGRESSION="${BONKLM_SEC_REGRESSION:-packages/core/benchmarks/security-regression/run-gate.mjs}"
  if [ -f "$SEC_REGRESSION" ]; then
    case "$SEC_REGRESSION" in
      *.mjs | *.js) run_gate "security-regression" node "$SEC_REGRESSION" ;;
      *) run_gate "security-regression" bash "$SEC_REGRESSION" ;;
    esac
  else
    skip_gate "security-regression" "no security-regression script found at $SEC_REGRESSION"
  fi
  ...
  # one slips past the workspace-level scan. A missing scanner is a hard failure;
  # findings are redacted and evidence stays in a local, gitignored path.
  run_gate "tarball-secret-scan" bash scripts/scan-tarballs.sh

  # ---- Tarball deny-pattern scans (blocking) ------------------------------
  # The exported public-safe gate always checks packed consumer bytes for path,
  # allowlist, special-file, and generic internal-surface violations. Canonical
  # checkouts add the excluded operator policy as a second blocking overlay.
  run_gate "tarball-public-surface" node scripts/check-release-tarballs.js --workspace
  if [ -f tools/oss-export/scan-tarballs-deny.mjs ]; then
    run_gate "tarball-deny-scan" node tools/oss-export/scan-tarballs-deny.mjs
  fi

  # ---- Supply-chain gates (blocking) -------------------------------------
  # Default-install scope: runtime/optional dependencies and non-optional peers
  # are blocking. Connector SDK peers are explicitly optional, so consumers audit
  # the host SDK versions they choose rather than silently auto-installing them.
  ...
  # ---- Release container -------------------------------------------------
  # Build from the root workspace context, then assert the production image
  # is version-aligned, runs as the non-root user, and reaches /healthz.
  run_gate "container-build" pnpm docker:build
  LOCAL_IMAGE="ghcr.io/blackunicornsecurity/bonklm-server:${VERSION}"
  IMAGE_PLATFORM="$(docker image inspect --format '{{.Os}}/{{.Architecture}}' "${LOCAL_IMAGE}" 2>/dev/null)"
  run_gate "container-smoke" pnpm docker:smoke -- "${LOCAL_IMAGE}" "${VERSION}" "${IMAGE_PLATFORM}"
