...
> See `AGENTS.md` (canonical). Generated for Claude Code from the same rule source.
...
sign-off (DSP-high finance-namespace data never enters a model; classified back-office / special-project namespaces excluded); **no
train-on-own-output without a held-out, human-verified eval set** (collapse/poisoning guard); offensive
