...
> **LEGACY / PRIVATE IN v1.0.0.** The `@blackunicorn/bonklm-openclaw` package is preserved in this
> repository for migration reference, but it is marked `private:true` and is not part of the v1.0
> publish set. Existing pre-v1 integrations should migrate to framework-native middleware (Express,
> Fastify, NestJS, Hono, Elysia, Next.js) instead.
