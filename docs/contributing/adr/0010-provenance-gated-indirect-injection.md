...
2. **Laundering (Home-E).** Even with a connector-boundary scan, an agent can read a poisoned tool
   result, **paraphrase** it into benign-looking prose, and persist the paraphrase to memory. The
   laundered surface text matches no content pattern; the poison survives into trusted memory and is
   read back, with authority, on a later turn.
