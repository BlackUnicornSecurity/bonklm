...
- **Separate from `tools/check-workspace-policy.js`.** That gate decides which `tools/*` packages
  are publishable; this gate consumes that Tier-B classification and enforces the OSS/EE boundary.
  License tiers here are always called "OSS/EE" (or "Apache/BSL"), never "Tier A/B", to avoid a
...
- Private Tier-A `tools/*` packages are governed by `tools/WORKSPACE-POLICY.md` and remain outside
  this guard. Explicitly publishable Tier-B tools are public OSS roots, so this guard classifies
  their licenses, permits only the recognized permissive `Apache-2.0` and `MIT` identifiers, and
  rejects imports or runtime dependencies on EE packages.
