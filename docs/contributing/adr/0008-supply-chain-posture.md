...
> floors" — and (2026-08-17) — see "Amendment: channel promotion heals unpromoted prerelease
> `latest` values" below. Scope: how BonkLM measures and attests the integrity of what it publishes
> to npm. Authority: maintainer decision. Relates to ADR-0006 (dual-license model) and ADR-0007
> (OSS↔EE boundary guard).
...
## Amendment: channel promotion heals unpromoted prerelease `latest` values (2026-08-17)

The dist-tag policy above promises a monotonic, recoverable channel transaction. That promise is
about values **this tooling promotes**. npm itself writes `latest` on a package's first publish
regardless of `--tag`, so a family that grows a new package acquires a prerelease `latest` the
transaction never created. Treating those values as promoted state made the stable-channel guard
fail closed against registry state the lane can neither have produced nor repair.

Amended posture:

- The stable/prerelease channel rule binds the **promotion target** (unchanged: a prerelease never
  moves `latest`; a stable never moves `next`).
- An existing prerelease value on `latest` is classified as unpromoted garbage: it does not count as
  a promoted newer release, does not satisfy the promotion, does not trip the rollback refusal, and
  is overwritten by the promotion's dist-tag adds. Each overwrite is logged with the package name
  and value.
- A stable value on `next` has no npm side-effect origin — it can only mean an external writer — and
  stays a hard stop.
- The mixed/rollback refusal is unchanged for channel-fitting values: a fitting newer value anywhere
  in the family still refuses the promotion.
- The failure rollback restores the exact prior channel state, including unfit values (restore
  fidelity over opportunistic healing); the next successful promotion heals them.
- The post-promotion dist-tag read-back retries with backoff before triggering a rollback — both a
  still-stale read (registry reads lag the writes just performed) and a transiently failing read. A
  channel refusal observed mid-read-back (a concurrent or external writer) aborts into the rollback
  immediately; it is a decision, not a read failure.
