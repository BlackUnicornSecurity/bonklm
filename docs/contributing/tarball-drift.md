...
## Root-level stale-emit guard

The Tarball Drift CI job (and the full quality gate) also run `pnpm check:stale-dist`
(`tools/check-stale-dist.js`) between the build and the snapshot tests. Every package builds with
plain `tsc`, and `tsc` never deletes: renaming or deleting a `src/` file orphans its old emit in
`dist/`, and because each package ships `files: ["dist", …]` the orphan rides the tarball to npm
consumers. The gate asserts the src↔dist bijection — every `dist/` file must have a `src/` twin —
and fails naming the class and the clean-rebuild fix
(`rm -rf packages/<pkg>/dist && pnpm --filter <pkg> run build`) before the (opaque) snapshot diffs
below it. If it trips, the accompanying tarball-drift snapshot failure is the same root cause: fix
the dist, do **not** regenerate the snapshot.
