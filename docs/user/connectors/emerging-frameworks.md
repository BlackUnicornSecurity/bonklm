...
The package manifests in this guide are pinned at `1.0.13`.
...
The Construct A shadow-log read that replaces the user-authored-memory bucket is in the v0.5.0
backlog and is NOT yet in Phase-2.
...
| `createElizaOSDrizzleShadowLogStorage` / `assertRoomAccess` / `verifyAndReadAuthenticatedMessages` | Shadow-log primitives.                                      |
...
- Tool / action call injection protection.
- Structured content handling (arrays, images, mixed).
