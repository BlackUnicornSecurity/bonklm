...
| Fastify   | `@blackunicorn/bonklm-fastify` | `fastify ^5.8.5`           | STABLE |
| NestJS    | `@blackunicorn/bonklm-nestjs`  | `@nestjs/common ^11.1.18`  | STABLE |
| Hono      | `@blackunicorn/bonklm-hono`    | `hono ^4.12.34`            | STABLE |
| Elysia    | `@blackunicorn/bonklm-elysia`  | `elysia ^1.4.0` (optional) | STABLE |
| Next.js   | `@blackunicorn/bonklm-nextjs`  | `next ^16.2.11` (optional) | STABLE |
...
The package manifests in this guide are pinned at `1.0.14`. Edge bundles require Workerd
`nodejs_compat`.
...
The NestJS session-category parity gap is now closed, matching the rest of the core validators —
...
Three surfaces for Next.js `^16.2.11`:
...
- Production mode toggle for generic error messages.
