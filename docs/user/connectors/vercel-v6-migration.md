...
`tool-input-delta`, `reasoning-delta`, etc.) — track follow-up PRs.
...
- `messagesToTextLegacy(messages)` — alias for `messagesToText`. When the v3/v4 type drop lands in a
  future PR, `messagesToText` will switch to `ModelMessage` and `messagesToTextLegacy` will retain
  the original `CoreMessage` shape.
...
The full acceptance criteria include items NOT yet shipped in Phase-1 — these will land as follow-up
PRs against the same package:
