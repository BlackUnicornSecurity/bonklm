...
| MCP SDK                                          | `@blackunicorn/bonklm-mcp`                 | `@modelcontextprotocol/sdk ^1.25.2`                                 | STABLE |
...
The package manifests in this guide are pinned at `1.0.14`. See
...
>
> The v1.0-RC1 API freeze removed the `messagesToTextLegacy` alias; rename to `messagesToText`.
...
Node-only. Wires the `AudioStreamValidator` into the four AC-mandated voice-agent hooks:
