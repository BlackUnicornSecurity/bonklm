...
> **Status**: Authoritative for all connectors from v0.5.0 onward. **Audience**: connector authors
> writing or extending packages under `packages/*-connector/`.

This guide locks the canonical factory naming + signature shape for every BonkLM connector shipping
in v0.5.0+. Read it BEFORE writing or extending a connector. Every new connector's acceptance
criteria reference this guide; PR reviewers reject divergences.
...
- `wrapAgent(agent, engine, options?)` — OpenAI Agents SDK.
- `wrapMCPClient(client, engine, options?)` — Vercel MCP.
...
- `createGuardedLanceTable(table, options)` — `@blackunicorn/bonklm-lance`.
- `createGuardedClient(client, options)` — `@blackunicorn/bonkdrant`.
- `createGuardedClient(client, options)` — `@blackunicorn/bonklm-pinecone`.
- `createGuardedClient(client, options)` — `@blackunicorn/bonkviate`.
...
rename — semver). Style-guide ADR amended after the architect-lane audit flagged three connectors
already shipping this shape undocumented.
...
- `honoGuardrails(engine, options?)` — Hono.
- `bonkMiddleware(engine, options?)` — Vercel AI SDK (legacy name kept for backwards compat; new
  framework middleware connectors follow `<framework>Guardrails`).
...
- `bonklmPlugin({ engine, validators, ... })` — ElizaOS.
...
- `withBonkLM(options)` — Trigger.dev v3/v4. Returns `{ middleware, onFailure }`; survives CRIU
  checkpoint/resume via the SDK's `locals` registry.
...
- Hoist resolution (engine + salted keyFn + base options) to factory scope so all bindings share the
  same closure (defeats the per-invocation cache-miss footgun documented in the Inngest connector).
...
```ts
// PRIMARY EXAMPLE — wrapMem0Client (ships in v0.5.0)
//
...
export function wrapMem0Client(
  ...
  // The fail-safe only catches null/undefined at engine construction;
...
> establishes. A future ESLint rule `bonklm/no-bare-catch-of-connector-validation-error` enforces
> this at CI; today it is a reviewer-checked discipline.
...
6. **Cross-reference the internal audit checklist** when writing acceptance criteria that touch
   `runtime.bonklm.*`, the probe-await semantics, or the AsyncLocalStorage call-context isolation.

---

## Documented pre-guide deviations
...
entrypoint so consumers can pick. The legacy entrypoints become `@deprecated` in v0.5 and are
removed in v1.0 (matching the HMAC sync-removal cadence).

| Connector     | Legacy shape                                                                     | Canonical shape                                              | Sunset                            |
| ------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------ | --------------------------------- |
...
additions. Both legacy entrypoints emit `@deprecated` warnings in v0.5 and throw `TypeError` at
v1.0.

### Later connector exceptions

Two connectors ship with a deliberate deviation from the strict shape #3 default. Both are
acknowledged retroactively as part of the shape-#5 ADR amendment:
...
This ADR establishes this guide as authoritative. From v0.5.0 onwards:

1. **Every new connector's acceptance criteria** reference this guide explicitly ("MUST follow shape
   #1/#2/#3/#4 per `docs/user/connector-style-guide.md`").
...
- `docs/user/error-codes.md` — `ConnectorValidationError` codes.
- `tools/WORKSPACE-POLICY.md` — Tier A internal vs Tier B publishable workspace tooling.
- Internal roadmap — full v0.4 → v1.0 roadmap referencing this guide.
...
| Date       | Change                                                                                                                                                                                                                                                                                              |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-22 | Initial authoring. 4 canonical sub-conventions, Mem0 PRIMARY multi-surface example, Zep ILLUSTRATIVE footnote, pre-guide deviations table, documented exceptions, sunset clauses.                                                                                                                   |
| 2026-05-23 | Added shape #5 (Task-options bindings factory) for Trigger.dev. Added later-connector deviations table with retroactive Inngest reclassification (shape #4, host-constrained) + Trigger.dev shape #5 row. Per-SDK idiomatic naming permitted for shape #5.                                          |
| 2026-05-23 | Added shape #2b (Vector-database sub-client wrap with validators-in-opts) covering `createGuarded<Subject>(subject, options)` for LanceDB, retroactively documenting the qdrant / pinecone / weaviate convention. Audit-loop architect lane flagged three undocumented uses; ADR amended pre-merge. |
