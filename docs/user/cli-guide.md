...
> Last updated: 2026-08-14. Applies to `@blackunicorn/bonklm` `1.0.16`.
...
1. Detect project dependencies (`detectFrameworks`) — every package any connector declares.
...
4. Present a `multiselect` of every registered connector. Detected connectors sort first and start
   pre-selected; each row shows its category.
5. For each selected connector, re-use existing env credentials or prompt via `@clack/prompts`
   ...
   `sk-ant-`. Env vars a connector declares optional (`OLLAMA_HOST`, the voice-webhook provider
   secrets) accept an empty answer and are then skipped rather than written.
6. Test each connector with a 10s timeout (`testConnectorWithTimeout`). Catalog connectors use an
   `installed` probe (declared credentials present and well-formed, and the connector package or one
   of its peer SDKs present in `package.json`) or a `tcp` probe against a declared local port.
...
The connector registry covers **every publishable BonkLM connector package** — `openai`,
`anthropic`, `ollama`, `express` and `langchain` are hand-written reference connectors; the rest are
declared in the connector catalog. Run `bonklm status` for the live list, which is also echoed under
"Available connectors:" when an invalid ID is supplied.

The three `connector` subcommands below all validate `<id>` against this registry-backed set through
a shared format guard, so the list cannot drift between commands. A test
(`registry.workspace.test.ts`) fails the build if a published connector package is missing from the
registry, or if the registry names a package that is not published.

What "configure" means varies by connector, and the wizard does not pretend otherwise:

- Connectors that own a documented environment variable (OpenAI, Anthropic, Mistral, Google GenAI,
  Hugging Face, Pinecone, Turbopuffer, Zep, Daytona, the Groq/Cerebras/Together wrappers, the
  self-hosted server) prompt for it and write it to `.env`.
- Every other connector is configured in code against a client you have already authenticated. For
  those the wizard detects, tests and prints a snippet, and writes nothing to `.env`.

See [`connector-descriptors.md`](./connector-descriptors.md) for the descriptor schema and for how
to add a connector. For the published package surface see
[`package-matrix.md`](./package-matrix.md).
...
Diagnose the local BonkLM contributor environment. Source:
...
**must** be ≥ 32 UTF-8 bytes — the server refuses to start otherwise. HMAC authenticates the body
but does not encrypt it, so all non-loopback production traffic must use HTTPS.
...
| `BONKLM_PRODUCTION_MODE`         | `true`     | Strip validator reasons from HTTP responses. CLI defaults to `true`; opt out explicitly for debugging.   |
| `BONKLM_TRUSTED_TLS_TERMINATION` | `false`    | Required when production mode binds a non-loopback host; asserts a trusted HTTPS proxy or load balancer. |

**Quick start:**

```bash
export BONKLM_HMAC_SECRET=$(openssl rand -base64 32)
export BONKLM_HOST=127.0.0.1
npx @blackunicorn/bonklm-server
# bonklm-server listening on http://127.0.0.1:4123
```

For a production non-loopback bind, place the service behind an HTTPS terminator and set
`BONKLM_TRUSTED_TLS_TERMINATION=true`. The executable refuses that bind without the assertion.
...
**`bonklm-server: BONKLM_HMAC_SECRET env var REQUIRED and MUST be >= 32 UTF-8 bytes`** — generate
