...
Wire BonkLM's `createRetrievedDocValidator` into a `pg`-based pgvector search so retrieved RAG hits
flow through prompt- injection / secret / PII scanning before reaching the LLM. No dedicated
`pgvector-connector` package is needed — pgvector is just Postgres + the `vector` extension, so the
standard `pg` driver combined with a thin wrapper carries the full validator surface.
...
ALSO run `createComposedContextValidator` on the joined string. The wake-up attack class — three
benign chunks that combine into an injection — is invisible to a per-doc scan.
