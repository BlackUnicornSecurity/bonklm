# BonkLM Public API Surface (v1.0.13 freeze)

Audit baseline for the v1.0-RC API freeze, extended across post-rc.3 hardening. This document
enumerates what is **PUBLIC** (frozen for v2.0) vs **INTERNAL** (may change without notice in v1.x).
...
- `rescanLaunderedProvenance` (function) + `ProvenanceRescanResult` (interface) + the
  `RAW_UPSTREAM_MATCH_REDACTED` marker — the laundering re-scan consumer that
...
- **`validateWithTimeoutSecure` (function)** — canonical timeout primitive. ALL connector authors
  MUST use this instead of rolling their own AbortController-based timeout (the AbortSignal does not
  propagate to `engine.validate()`).
...
- **`TimeoutSentinelShape` (interface)** — minimum shape every `R` must satisfy
  (`{ allowed: boolean }`). The `allowed: false` invariant is the security boundary on timeout.
  Connectors that surface `GuardrailResult` / `EngineResult` / wrapped shapes automatically satisfy
...
- `BonklmTraceSurface` (locked vocab)
...
- **`validatorInstance`** + **`loggerInstance`** + **`attackLoggerInstance`** added to match the
  canonical object-shape `Validator` / `Logger` / `AttackLogger` interfaces (the older `function`
  rule rejects class instances). Use these for connector middleware schemas that accept
  user-supplied validators / loggers.
- **`OptionalRule` semantics**: short-circuits on `undefined` only. Explicit `null` flows into the
  inner rule (typically rejected by `TypeRule`). Callers should omit the key or pass `undefined` to
...
- `redactSecrets` (function) — finding/telemetry-egress credential redactor (common helper); barrel-
  reachable but tactical, may be re-organised or merged with the CLI error redactor.
...
| Symbol                      | Reason                                                                                                                              | Removal                                                                                                               |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `messagesToTextLegacy`      | Replaced by `messagesToText`                                                                                                        | v1.0-RC1                                                                                                              |
| `GuardrailsCallbackHandler` | Replaced by `wrapLangChain`                                                                                                         | v1.0-RC1                                                                                                              |
| sync `validateToken`        | Replaced by async `validate`                                                                                                        | v1.0-RC2                                                                                                              |
