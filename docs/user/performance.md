...
`allowEmptyForTesting: true` is set (Story 0.1 fail-safe) — this is a correctness gate, not a
...
- **`composed-context.bench.ts`** — `createComposedContextValidator` hot path at the 32 KB soft cap.
  Stated target (R2-11): P99 < 200 ms on 32 KB input, both benign and attack variants. Two-pass
...
success per Sprint 34 architect closure):
...
- `\bNaN` or `\bInfinity` in benchmark output fails the build (broken measurement — Sprint 33
  lessons-learned closure).
- `[ERROR] Error in (guard|validator)` in benchmark output fails the build (engine catch-site fired
  during the bench is an interface or contract violation — Sprint 33 SecretGuard-context regression
  class).
...
by the tracer). The R2-10 locked attribute vocabulary on each span:
