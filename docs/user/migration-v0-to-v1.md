...
> the full v1.0 line. v0.7 consumers upgrading directly to v1.0.0 need to address only the sections
> marked as applicable to their current version.
...
`undefined` AND `null`. It now only short-circuits on `undefined`; explicit `null` flows into the
...
timeout. It now rejects `0` to align with `validateWithTimeoutSecure`, which throws `TypeError` on
`timeoutMs <= 0`.
...
there is no "disable" sentinel. Sub-ms granularity is enforced because race-with-timeout requires a
positive integer.
...
silently turned the rule into an UNBOUNDED rule (accepting negative numbers). The `min` argument is
now always honoured.
...
is the **telemetry / hook _surface_ vocabulary** (the `surface` field on hooks and OTel spans;
frozen at v1.0):
...
15 new connector packages shipped between v0.5.0 and v0.6.0. These are additive — existing consumers
see no change. New packages:
...
- Per-row verification happens **inside** the body parser **before** `JSON.parse` to preserve the
  route-enumeration-oracle closure.
...
`cloudflare-agents-connector` reference config. We re-audit this pin every release prep to avoid the
late-2024 `nodejs_compat` semantic drift.
...
The sandbox-attack-corpus graduation gate **PASSED** at 100% recall / 0% false-positive / 100%
precision. The corpus is hash-pinned at commit `4f8ea3f`:
...
Multilingual prompt-injection Pass 2 (7 additional languages: Indonesian, Turkish, Persian,
Vietnamese, Thai, Polish, Dutch) was retired after a prolonged stall against limited maintainer
capacity + no native-reviewer pipeline. The current shipping corpus covers 12 languages (es / fr /
de / pt / it / zh / ja / ko / ru / ar / bn / ur) — see `known-limitations.md` §25.

Multilingual Pass 2 is **deferred, CONDITIONAL** — will reopen only when a native-reviewer pipeline
materialises.

If you depend on detection in a language outside the shipping 12, either:
...
const r = bonklmTrace(await validator.validate(input), {
  ...
  surface: 'text_input' // locked vocabulary
});
```
...
## §14 — express-middleware: unparsable bodies now fail closed (v1.0.1)

The default body extractor no longer scans a placeholder literal when a request body cannot be
serialized (circular references, BigInt values from non-JSON body parsers). Two behavior changes:

1. **`unparsableBodyPolicy` defaults to `'block'`**: an unparsable body is rejected with a
   fail-closed result instead of validating the inert string `'[Unparsable body]'` (which always
   passed — a fail-open path). Set `unparsableBodyPolicy: 'scan-literal'` to restore the legacy
   behavior; this is not recommended for security-relevant deployments.
2. **Bodies previously collapsed to `'[Circular]'` are now scanned as real JSON**: the root-object
   replacer bug was fixed, so bodies that used to shrink to an inert literal are now validated with
   their actual content. Expect new (correct) blocks on pathological payloads.

The failure signal moved out-of-band (a thrown `UnparsableBodyError` exported from
`@blackunicorn/bonklm-express-middleware`): a body that legitimately _contains_ the text
`'[Unparsable body]'` is no longer misclassified as unparsable.

## §15 — bonklm-server: replay protection hardening (v1.0.1)

The seen-signature replay cache is now capacity-bounded fail-closed (HTTP 503
`replay_cache_exhausted` at capacity, default raised to 100,000 entries), retains signatures past
the acceptance window, and is injectable (`replayCache` option / `BONKLM_REPLAY_CACHE_SIZE`) for
multi-replica deployments. A signature claimed by a request that later fails parsing (415/400) is
still consumed — mint a fresh timestamp per attempt. See the `@blackunicorn/bonklm-server` README
("Replay protection").
