...
- **Node.js** `>= 20.4.0` for consumers; use Node 22 or 24 for release development and CI.
- **pnpm** `9.x` (the project uses pnpm workspaces — `packageManager` field pins `pnpm@9.15.9`). npm
  is **not** supported because the workspace uses `workspace:*` protocol.
- **Git** for version control.
- **Docker** with Buildx, plus host `openssl` and `curl`, for the mandatory container build and
  smoke gates in `pnpm quality-gate`.
...
3. Build all packages in workspace dependency order:

   ```bash
   pnpm run build
   ```

   The private workspace root and the published core package deliberately have different package
   names. This lets pnpm model the real workspace graph and finish core before starting packages
   that resolve its published `dist/` exports. Keep workspace package names unique when adding or
   renaming packages.
...
// package.json
"simple-git-hooks": {
  "pre-commit": "gitleaks protect --staged --config .gitleaks.toml --redact --no-banner && pnpm format:check && pnpm lint && pnpm typecheck && bash scripts/check-version-pin.sh"
}
```

Before every commit this fails closed if gitleaks is unavailable or detects a staged secret, checks
Prettier and ESLint, runs `tsc --noEmit` across the workspace, and verifies the single-version pin.
...
- **Context first.** Read the relevant docs, locate the task in the project's planning and QA
  documentation, study the code you will touch and its callers, and review the project's
  lessons-learned log — _before_ writing code. No item in the plan for your task? Create one and
  align it first.
- **On doubt, search the knowledge base** (the project's lessons-learned log, the QA defects /
  decisions / risk register) before improvising.
...
2. Open a PR on the canonical development repository with:
...
1. PRs require the green server-side review status; maintainer approvals are not required.
2. Every PR receives the adversarial core-4 pass (senior developer, architect, red-team, security).
3. Address every review comment. Reviewers reject `lgtm` reviews that haven't flagged anything — the
   project deliberately maintains an adversarial review culture.
...
- Every PR receives the core-4 adversarial audit; all findings, including LOW, close before commit.
- The server-side review status is the merge gate; approval count remains zero.
- Never commit secrets, API keys, or credentials.
- Report security vulnerabilities **privately** to `info@blackunicorn.tech`, not via public GitHub
  issues.
...
Per `RELEASE-NOTES.md`, `packages/core/package.json`, and the package matrix, **all 52 publishable
package manifests and the private root metadata carry the same version**. The two private
tooling/legacy package manifests are outside the version-locked release family. The current release
line is **`1.0.15`**. The `.changeset/config.json` `linked` array enumerates all 52 publishable
package names. Changesets `linked` aligns the target version of family members that are present in a
release plan, but it does not add untouched members. Every changeset affecting a member of the
BonkLM linked family must therefore enumerate all 52 publishable packages at the same bump level;
tool-only releases outside that family are ignored by this gate. `pnpm run check:release-plan`
enforces the prospective plan before versioning. The two private packages
(`@blackunicorn/bonklm-openclaw`, `@blackunicorn/bonklm-wizard`) are excluded and both are listed
under Changesets `ignore`. The package matrix is the release-surface inventory. Regenerate the
linked list from the publishable manifests with (re-derive at HEAD rather than hand-editing — this
is how the list drifted stale before):
...
`pnpm run check:changeset-linked` locally to print the exact names that are missing or extra. The
adjacent `release-plan` job runs Changesets status and fails if an active family release omits a
publishable package or would produce more than one target version.
...
The private `@blackunicorn/bonklm-openclaw` and `@blackunicorn/bonklm-wizard` packages are listed
under `ignore` in changeset config and are **not** published from this monorepo.
...
A human-approved, published GitHub Release triggers `.github/workflows/publish.yml`; pushing a tag
alone never publishes, and direct Changesets publication is intentionally unavailable. Family
releases use `v<semver>` (for example `v1.0.1`). Independently versioned Tier-B tools use
`<unscoped-name>-v<semver>` (for example `eslint-plugin-edge-v0.4.1`), avoiding collisions with
family tags. The release body contains exactly one `Release-Scope: family` marker, or an explicit
Tier-B package name such as `Release-Scope: @blackunicorn/eslint-plugin-edge`. Coincidentally
matching versions never expand the scope. The release fans out only after shared build, test,
export, license, SBOM, secret, and shipped-advisory gates pass. Family preflight builds one
`linux/amd64` + `linux/arm64` OCI artifact, scans and smoke-tests both platforms, and retains their
SBOMs. That exact digest is staged, signed, and held in a private staging package while the exact
npm versions publish and their provenance is verified. Only then is the signed digest copied to the
exact public GHCR SemVer tag. npm alone moves `latest` or `next`, after saving a complete recovery
vector, and final reconciliation verifies exact npm bytes, signatures, provenance, and monotonic
channel state. GHCR intentionally has no mutable `latest`/`next` tags; consumers pull the exact
version. Tier-B releases publish only the explicitly named tool and its package-scoped SBOM; they do
not enter the container lane.

Before the first family publication, an organization administrator must pre-provision the
`bonklm-server` GHCR package through the separately reviewed package-provisioning procedure, set it
to **Public**, and verify anonymous access. This release workflow deliberately refuses a missing or
private public-package target; it does not bootstrap one. Retries refuse to retarget an existing
exact GHCR version.

Create protected `public-release` and recovery-only `public-release-recovery` environments before
enabling the workflow. Put `NPM_TOKEN` and `BONKLM_RELEASE_DENY_TERMS` in the release environment;
give the recovery environment only the narrowly scoped credentials it needs. The deny-term secret
supplies the private exact-tarball overlay, and the workflow fails closed when it is absent.
Announce a release only after the exact commit has a successful `bonklm/release-ready` status.

Family prerelease tags must be cut from a branch in Changesets `pre` mode
(`pnpm exec changeset pre enter rc`) and published as a GitHub prerelease. Tier-B prereleases are
independent of the family Changesets pre-state. Prereleases move npm `next`; stable releases move
npm `latest`. npm versions are registry-immutable; the workflow enforces immutability for the exact
GHCR tag and refuses to retarget it on retries.
...
The project keeps a running lessons-learned log of what went wrong and why. **Read it before
starting a non-trivial task**, and **append to it when you hit a non-obvious failure mode**.
...
- **Canonical development-forge issues** — bugs and feature requests.
- **Canonical development-forge discussions or PR comments** — questions and ideas.
- **Documentation** — `docs/user/`.
- **Security** — `info@blackunicorn.tech` (private).
