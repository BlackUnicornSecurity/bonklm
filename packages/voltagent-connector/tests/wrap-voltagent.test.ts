...
import { wrapVoltAgent, VoltAgentGuardrailBlockedError, type VoltAgentLike } from '../src/index.js';
...
describe('wrapVoltAgent — surface', () => {
  it('throws TypeError when agent missing generateText', () => {
    expect(() => wrapVoltAgent({} as VoltAgentLike, { engine: makeEngine() })).toThrow(TypeError);
  });

  it('throws TypeError when neither engine nor inputValidators supplied', () => {
    expect(() => wrapVoltAgent(makeMockAgent(), {})).toThrow(TypeError);
  });
...
describe('wrapVoltAgent — generateText input validation', () => {
  ...
  it('blocks attack prompt + fires onBlock with kind=inference provider=voltagent', async () => {
    ...
    await expect(wrapped.generateText({ prompt: attackText })).rejects.toBeInstanceOf(VoltAgentGuardrailBlockedError);
...
describe('wrapVoltAgent — generateText output validation', () => {
  it('blocks tainted output', async () => {
    ...
    const wrapped = wrapVoltAgent(agent, { engine: makeEngine() });
    await expect(wrapped.generateText({ prompt: benignText })).rejects.toBeInstanceOf(VoltAgentGuardrailBlockedError);
  });
