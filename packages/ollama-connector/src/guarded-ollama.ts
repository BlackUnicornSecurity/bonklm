...
export function createGuardedOllama(
  ...
  guardedClient.generate = async (opts: GuardedGenerateOptions): Promise<any> => {
    ...
    if (content) {
      ...
      if (outputBlocked) {
        // CWE-117 sweep: generate-path sister of chat-path
 ...
 * buffer semantics. The buffer-size cap is still enforced per chunk.
 */
async function* createBufferedValidatedChatStream(
  ...
  for await (const response of stream) {
    ...
    // also cap the retained-response count. Buffer mode holds one object
