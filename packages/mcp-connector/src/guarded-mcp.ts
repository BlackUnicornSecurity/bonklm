 ...
 *
 * Security Features:
 * - Tool call injection via JSON.stringify - schema validation
...
/**
 * Validates and sanitizes a tool name.
 *
 * @internal
 * @remarks
...
/**
 * Sanitizes tool name for validation content.
 *
 * @internal
 * @remarks
...
export function createGuardedMCP(client: Client, options: GuardedMCPOptions = {}): GuardedMCPClient {
  const {
    ...
    maxArgumentSize = DEFAULT_MAX_ARGUMENT_SIZE, // Default 100KB
    productionMode = process.env.NODE_ENV === 'production',
    validationTimeout = DEFAULT_VALIDATION_TIMEOUT, // Default 5s
  ...
  const validateToolCall = async (toolName: string, args: string): Promise<void> => {
    ...
    if (blocked) {
      ...
      // CWE-117 sweep — surfaced by integration test:
      ...
      // Sanitize at the throw site per the defensive-by-default
      // policy.
  ...
  const validateToolResult = async (toolName: string, resultContent: string): Promise<ToolCallResult | null> => {
    ...
    if (blocked) {
      ...
      // CWE-117 sweep — surfaced by integration test: this
      // is the SISTER site of the error-catch fallback filteredText
      // (~line 402) that wrapped. The sweep missed
      ...
      // terminal output. Per the defensive-by-default policy:
  ...
  const createGuardedWrapper = (): GuardedMCPClient => {
    return {
      ...
      async callTool(opts: ToolCallOptions): Promise<ToolCallResult> {
        ...
        // Validate tool name against allowlist and format
        validateToolName(name, allowedTools);

        // Validate argument size before processing
