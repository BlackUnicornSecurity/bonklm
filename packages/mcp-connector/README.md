...
| Feature                    | Description                                             |
| -------------------------- | ------------------------------------------------------- |
| Tool Name Validation       | Validates tool name format and checks against allowlist |
| Argument Schema Validation | Validates argument size to prevent DoS                  |
| Production Mode            | Generic errors in production to avoid info leakage      |
| Validation Timeout         | Uses AbortController for timeout enforcement            |
| Proper Logger Integration  | Uses `createLogger('console')` instead of raw console   |
