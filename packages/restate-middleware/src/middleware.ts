 ...
 *
 * Story 4.4 FINISH (Sprint 21): extended to cover both `Context`
...
export interface RestateCtxLike {
  ...
  /**
   * Story 4.4 FINISH: ObjectContext.key() returns the virtual object's
   * key. When present, it's mixed into the journal key so per-object
   * replay cache is isolated.
   */
  key?: () => string;
  /**
   * Story 4.4 FINISH: ObjectContext.get<T>(stateKey) for replaying
  ...
  /**
   * Story 4.4 FINISH: ObjectContext.set<T>(stateKey, value) — used
...
export interface RestateMiddlewareOptions {
  ...
  /**
   * Sprint 21 audit closure (architect C3 + code-reviewer C-3 +
   * security C-2): override the state key used to persist the last
...
/**
 * Restate block event. Sprint 21 audit closure (architect C1):
 * carries `kind: 'durable-exec'` + `runtime: 'restate'` so it is
...
// Sprint 21 audit closure (code-reviewer C-4 + security N-2):
// EXPERIMENTAL warn-once removed. Sprint 21 ships full ObjectContext
// + integration tests; the banner is no longer accurate.
...
export function withRestateGuardrails<Ctx extends RestateCtxLike, In, Out>(
  ...
  return async function wrappedHandler(ctx: Ctx, input: In): Promise<Out> {
    // Story 4.4 FINISH: incorporate ObjectContext key into journal
    // key so two virtual objects don't share replay state. Falls
    // back to base key when ctx.key() is not present (Context, not
    // ObjectContext).
    //
    // Sprint 21 audit closure (architect C4 + code-reviewer C-2 +
    // security C-1):
    ...
    // Story 4.4 FINISH: persist last-decision (ALLOW) summary so
    // operator dashboards can correlate handler runs against
    // validation outcomes. Sprint 21 audit closure: configurable +
...
/**
 * Sprint 21 audit closure (security C-1): sanitize ObjectContext key
...
/**
 * Sprint 21 audit closure (architect C3 + code-reviewer C-3 +
 * security C-2): resolve the last-decision state key from options.
