...
export function createGuardedAnthropic(
   ...
   *
   * routes through the shared `validateWithTimeoutSecure`
   * primitive from core/connector-utils — single canonical regression
  ...
  const validateInput = async (messages: MessageParam[]): Promise<void> => {
    ...
    if (blocked) {
      ...
      // cross-connector CWE-117 sweep: sanitize at dev-mode
  ...
  const createValidatedStream = (
    ...
    if (validateStreaming && streamingMode === 'buffer') {
      // Buffered full-stream validation (hold-back-and-release)
      // Max buffer size enforcement
      ...
      const apiCall = async (): Promise<Message | AsyncIterable<MessageStreamEvent>> => {
        ...
        try {
          ...
          if (isStreaming) {
            ...
            result = createValidatedStream(stream, runId);
          } else {
            ...
            if (content) {
              ...
              if (outputBlocked) {
                ...
                // transcript). Sanitize at the boundary
 ...
 *
 * @remarks
 * Implements incremental validation and buffer-size limiting.
 */
 ...
 *
 * The buffer-size cap is still enforced per chunk before accumulation.
 */
async function* createBufferedValidatedStream(
  ...
  // single buffer-exceeded path (text size OR retained-event count).
  ...
  try {
    for await (const event of stream) {
      ...
      if (event.type === 'content_block_delta' && event.delta?.type === 'text_delta') {
        ...
        // cap accumulated text BEFORE accumulating.
        if (accumulatedText.length + content.length > maxStreamBufferSize) {
      ...
      // also cap the retained-event count. Buffer mode holds one object
