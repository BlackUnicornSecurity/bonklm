...
export class GuardrailsService {
  ...
  /**
   * Validate content with timeout enforcement.
   *
   * @param content - The content to validate
   * @param context - Optional context string (regression: Use string context)
   * @returns Validation results
   */
  private async validateWithTimeout(content: string, context: string): Promise<GuardrailResult[]> {
    ...
    try {
      ...
      // are non-enumerable. Use the canonical primitive.
   ...
   * Respects production mode setting.
   *
  ...
  getErrorMessage(result: GuardrailResult): string {
    ...
    // the raw CR/LF forges phantom log lines.
  ...
  checkSessionEscalation(request: any): {
    ...
    // fastify fix. `SessionTracker.ts:321` embeds
    ...
    // logs, response bodies) inherit safety
