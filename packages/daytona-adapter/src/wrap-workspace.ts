/**
 * `wrapWorkspace(workspace, options)` for Daytona
 * ==============================================================================
 *
 ...
 * validateCode(search) + validateCode(replace).
 *
 * Inherits all Sprint 19 hardening patterns from e2b-adapter:
...
export function wrapWorkspace<W extends DaytonaWorkspaceLike>(workspace: W, options: DaytonaWrapOptions = {}): W {
  ...
  const wrappedFs: DaytonaFsLike = {
    ...
    replaceInFiles: async (filePaths: string[], search: string, replace: string, opts?: unknown) => {
      // replaceInFiles path + value double-validated.
