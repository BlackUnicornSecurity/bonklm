...
- e9c62ce: Fix: `guardrailGate` now fails the workflow terminally on BLOCK.
  ...
  (`type: 'TemporalGuardrailBlockedError'`) so a BLOCK decision deterministically fails the
  workflow. A client awaiting the workflow receives a `WorkflowFailedError` whose `.cause` is that
  `ApplicationFailure`; the guardrail diagnostics (`validatorName`, `category`, `severity`,
  `reason`) are carried in `details[0]`, and the `TemporalGuardrailBlockedError` instance is
  attached as the failure `cause` for direct in-process callers. The `TemporalGuardrailBlockedError`
  class remains a public export.
