...
describe.skipIf(!INTEGRATION_ENABLED)('Temporal TestWorkflowEnvironment — real cluster + worker + workflow', () => {
  ...
  beforeAll(async () => {
    const executablePath = process.env.BONKLM_TEMPORAL_TEST_SERVER;
    if (!executablePath) throw new Error('BONKLM_TEMPORAL_TEST_SERVER must identify a verified test-server binary');
  ...
  it('ALLOW path: workflow completes with the processed string', async () => {
    // Sprint 35 hardening (code-reviewer HIGH): input picked so
