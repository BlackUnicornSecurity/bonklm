/**
 * Story 3.7 — `validateExtractedText` helper for DIY parsers
 * ===========================================================
 *
...
/**
 * Sprint 21 audit closure (architect N5 + security C-4): policy for
 * oversize documents.
...
export interface ValidateExtractedTextOptions extends DocumentIngestWrapOptions {
  ...
  /**
   * Sprint 21 audit closure (architect N1): per-call byte cap. Default
   * {@link MAX_EXTRACTED_TEXT_BYTES} (1 MB). Operators handling
   * legitimately-large legal/medical documents pass a higher value.
   */
  maxBytes?: number;
  /**
   * Sprint 21 audit closure (architect N5 + security C-4): oversize
...
export interface ValidateExtractedTextResult {
  ...
  excerpt?: string;
  /**
   * Sprint 21 audit closure (security C-4): `true` when validation
  ...
  skipped?: boolean;
  /**
   * Sprint 21 audit closure (architect N5): `true` when the document
...
export async function validateExtractedText(
  ...
  // Sprint 21 audit closure (BLOCK code-reviewer + security B-1):
