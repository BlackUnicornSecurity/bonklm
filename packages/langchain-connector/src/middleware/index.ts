/**
 * LangChain.js 1.0 Middleware Migration
 * =================================================
 *
 ...
 * `@langchain/core@0.3.x` ship as Phase-2+.
 *
  ...
  if (covers('text_input')) {
    middleware.beforeModel = async state => {
      ...
      if (!r.allowed) {
        ...
        // CWE-117 sweep: sanitize `r.reason` at dev-mode
        // throw boundary.
  ...
  if (covers('text_output')) {
    middleware.afterModel = async (_state, response) => {
      ...
      if (!r.allowed) {
        ...
        // CWE-117 sweep: same shape on output leg.
        throw new ConnectorValidationError(
  ...
  if (covers('tool_call')) {
    middleware.wrapToolCall = async (toolCall, next) => {
      ...
      if (!r.allowed) {
        ...
        // CWE-117 sweep: same shape on tool-call leg.
        throw new ConnectorValidationError(
...
export interface WithRetrieverGuardrailsOptions {
  ...
  productionMode?: boolean;
  /**
   * per-document validation timeout in milliseconds.
   *
   ...
   * budget across other connectors.
   */
  validationTimeout?: number;
}
...
export function withRetrieverGuardrails<TRetriever extends BonklmRetrieverLike>(
  ...
  // silently hang the retriever invoke call (same regression
  // closed across all other connectors). Default 5000ms.
    ...
    invoke: async (input: string, ...rest: unknown[]) => {
      ...
      for (const d of docs) {
        ...
        if (r.allowed) {
          ...
          // CWE-117 sweep: sanitize `r.reason` at log-meta
      ...
      // RetrievedDocValidator 'drop' mode default).
...
export async function bonklmLangGraphNode(
  ...
  // wrap engine.validate
  // in validateWithTimeoutSecure so the LangGraph node honours the
  // timeout even when callers don't supply their own outer timeout. Default
  // 5000ms matches the canonical budget.
  ...
  if (!r.allowed) {
    // CWE-117 sweep: sanitize `r.reason` at dev-mode throw
