/**
 * Story 3.4 — Vapi webhook handler
 * =================================
 *
  ...
  return async function vapiHandler(req: WebhookRequest): Promise<WebhookResponse> {
    try {
      ...
      if (!hmacResult.valid) {
        ...
        // Sprint 19 Story 3.4 audit closure (security C-1 + architect
      ...
      try {
        ...
        // Sprint 19 audit architect N4: post-auth parse failures are
...
function safeStringify(args: unknown): string {
  try {
    ...
    // Sprint 19 Story 3.4 audit code-reviewer-C5 closure: empty string
