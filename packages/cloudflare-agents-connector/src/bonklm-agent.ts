/**
 * `BonklmAgent<Env, S>` extends `Agent<Env, S>`
 * ==========================================================
 *
 * Cloudflare Agents (Durable Object) wrapper. Subclass-or-mixin
 * pattern matches LiveKit's `BonklmAgent extends Agent`
...
export function withBonklmAgent<S, Base extends AgentClassLike<S>>(BaseAgent: Base, config: BonklmAgentConfig): Base {
  ...
  // Sprint 22 hardening (security + code-reviewer BLOCK-1 +
...
/**
 * Find the base-class `sql` property (or any
