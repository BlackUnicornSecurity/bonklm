...
describe('ChromaDB Connector', () => {
  ...
  describe('Edge Cases - Input Validation', () => {
    it('clamps zero nResults to 1 before calling the collection (family parity)', async () => {
      const mockCollection = createMockCollection();
      const guarded = createGuardedCollection(mockCollection, { validators: [noOpValidator()] });

      // Family parity: chroma now uses the shared `normalizeLimit`, so nResults=0 clamps to
      // the lower bound 1 (consistent with weaviate/qdrant/pinecone) instead of the
      // old chroma-only `options.nResults || 10` default. Non-vacuous: assert the
      // clamped value actually reaches the collection.
      const result = await guarded.query({
        queryTexts: ['test'],
        nResults: 0
      });
      ...
      expect(result.documents).toBeDefined();
      expect(mockCollection.query).toHaveBeenCalledWith(expect.objectContaining({ nResults: 1 }));
    });

    it('clamps negative nResults to 1 instead of throwing (family parity)', async () => {
      const mockCollection = createMockCollection();
      const guarded = createGuardedCollection(mockCollection, { validators: [noOpValidator()] });

      // Family parity: chroma previously threw `nResults must be between 1 and N` on an
      // out-of-range nResults; it now clamps to [1, maxNResults] like the other
      // vector-DB connectors. -5 -> 1.
      const result = await guarded.query({
        queryTexts: ['test'],
        nResults: -5
      });
...
describe('ChromaDB Connector — CWE-117 batch-block reason sanitization (security regression)', () => {
...
describe('ChromaDB Connector — CWE-117 query/add reason sanitization is load-bearing (ADR-0001)', () => {
  ...
  // `result.reason` sinks in src/guarded-chroma.ts. The sibling security regression block
