...
- ef1bd45: feat(mastra): scan inbound tool results for indirect prompt-injection by default
  ...
  the general agent-output and streaming paths are unchanged, so `tool_result`-surface patterns
  never fire on ordinary LLM output. Gated by the existing `validateToolResults` option (default
  `true`).
