 ...
 * text — either reduced to a fixed placeholder (e.g. `image` → '[Image]',
 * `data` → '[Data]') or dropped to '' (an unrecognized part `type`). The
 * indirect-injection arm therefore never inspects that channel; surfacing the
 * tally turns the silent pass into an operator signal (mirrors the MCP
 * connector's uninspectable-blob telemetry, PR #146).
 *
 * @internal
 */
 ...
 */
export function messagesToTextWithTelemetry(messages: GenkitMessage[]): ReducedMessages {
 ...
 * @internal
 */
function reduceMessage(m: GenkitMessage): PartReduction {
 ...
 */
function reduceContentPart(part: GenkitContentPart): PartReduction {
  switch (part.type) {
    case 'text':
    ...
    case 'toolResponse': {
      ...
      return { text: toolRespParts.join('\n'), reducedKinds: [] };
    }

    case 'image':
      // Image parts are not validated as text (checked elsewhere). The '[Image]'
 ...
 * @internal
 */
function reducedKindLabel(part: GenkitContentPart): string {
