...
const guardedOpenAI = createGuardedOpenAI(openai, {
  ...
  // Streaming validation (regression)
  validateStreaming: true,
  streamingMode: 'incremental', // or 'buffer'

  // Max buffer size for streams (regression)
  maxStreamBufferSize: 1024 * 1024, // 1MB

  // Production mode for generic errors (regression)
...
Both modes enforce `maxStreamBufferSize` (regression).

## Multimodal Content

The connector properly handles complex message content types (regression):
