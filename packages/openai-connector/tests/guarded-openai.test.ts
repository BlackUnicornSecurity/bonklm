...
describe('OpenAI Guarded Wrapper', () => {
  ...
  describe('regression: Complex Message Content Handling', () => {
  ...
  describe('Streaming Validation (regression, regression)', () => {
    ...
    it('should terminate stream on validation violation (regression)', async () => {
      let callCount = 0;
      const mockStream = {
        async *[Symbol.asyncIterator]() {
          yield { choices: [{ delta: { role: 'assistant' } }] };
    ...
    it('should enforce max buffer size (regression)', async () => {
  ...
  describe('Advanced Streaming Validation Tests', () => {
    ...
    describe('Incremental vs Buffer Mode Differences', () => {
      ...
      // regression: buffered full-stream validation (hold-back-and-release).
      ...
      describe('buffer mode (hold-back-and-release)', () => {
        ...
        it('enforces maxStreamBufferSize (regression) by releasing nothing on overflow', async () => {
          const mockStream = {
            async *[Symbol.asyncIterator]() {
        ...
        it('caps the retained-event count on a zero-text structural-event flood (regression)', async () => {
          const mockStream = {
            async *[Symbol.asyncIterator]() {
