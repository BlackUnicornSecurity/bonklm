...
export interface BonklmPluginOptions {
   ...
   * to `runStartupProbe` (see `ProbeOptions.fetchImpl`). @default the
   * global `fetch`.
   *
   ...
   * monkey-patching `globalThis.fetch`. The probe still targets ONLY the
   * hardcoded loopback literals — `fetchImpl` cannot redirect it off-host
   * — and the options object is frozen at plugin construction, so this is
   * consumer-config trust (like `envBindings`), not an attacker surface.
