/**
 * Construct B (Phase-2) — `runtime.bonklm.wrapMemory`
 * =============================================================
 *
...
function assertMemoryWriteAllowed(
  ...
  if (memory.tableName === 'messages' && computedSource !== 'authenticated' && computedSource !== 'agent_internal') {
    // integration-test surfaced site: meta-field `caller`
  ...
  if (memory.tableName === 'messages' && callerPluginName !== undefined) {
    ...
    if (typoCheck.exactMatch) {
      ...
      // connector CWE-117 sweep: `callerPluginName` arrives
      ...
      // integration-test surfaced site: meta `caller` field
...
/**
 * Backwards-compat re-export. Phase-1 callers used
