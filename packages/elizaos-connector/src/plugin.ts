...
import type { BonklmPluginOptions, IAgentRuntimeLike, PluginLike, PluginLoadContext } from './types.js';
...
export function bonklmPlugin(options: BonklmPluginOptions = {}): PluginLike {
  ...
  return {
    ...
    async init(context: PluginLoadContext): Promise<void> {
      ...
      // Phase-2 step 4: shadow log auto-wire + acknowledge
      // deprecation warning.
        ...
        if (typeof runtime.on === 'function') {
          ...
          runtime.on('MESSAGE_RECEIVED', async (...handlerArgs: unknown[]) => {
            try {
              ...
              const sourceTrust = await classifySourceTrust(runtime, event);
