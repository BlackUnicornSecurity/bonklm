/**
 * `wrapSandbox(sandbox, options)` for E2B
 * ====================================================
 *
 ...
 *     → validatePath(path).
 *
 * **EXPERIMENTAL**: package.json carries
 * `experimental: true`. Recall-benchmark gate at v0.7 promotes to
 * stable contingent on corpus.
 *
...
// experimental banner removed.
// corpus gate passed (recall 100% / FPR 0% / precision 100%).
...
export function wrapSandbox<S extends E2BSandboxLike>(sandbox: S, options: E2BWrapOptions = {}): S {
  ...
  // Sprint 19 hardening (architect B1 + security B-1 + code-reviewer
  ...
  const wrapped: E2BSandboxLike = {
    commands: {
      run: async (commandOrBinary: string, argsOrOpts?: string[] | unknown, opts?: unknown) => {
        ...
        if (Array.isArray(argsOrOpts)) {
          return sandbox.commands.run(commandOrBinary, argsOrOpts, actualOpts);
        }
