...
export function createGuardedClient(
  ...
  const validateObjects = async (
    ...
    // batch validator path. Replaces the per-object loop when
  ...
  return {
    ...
    async query(queryOptions: WeaviateQueryOptions): Promise<GuardedWeaviateResult> {
      ...
      // Step 5: limit normalization — shared clamp to [1, maxLimit] (vector-DB family
      // parity: weaviate now uses the same `normalizeLimit` as qdrant/pinecone/chroma
      // instead of an inline copy). `DEFAULT_QUERY_LIMIT` is weaviate's own re-export (= 10).
      const limit = normalizeLimit(queryOptions.limit, { max: maxLimit, fallback: DEFAULT_QUERY_LIMIT });
