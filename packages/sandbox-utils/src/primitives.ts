 ...
 *
 * when called without an explicit `wrapperKey`
...
export async function* wrapStream<T>(
  ...
  // Sprint 19 hardenings (code-reviewer BLOCK + architect B1 +
  ...
  for await (const chunk of input) {
    if (typeof chunk !== 'string') {
      ...
      // in README — binary content is NOT validated;
