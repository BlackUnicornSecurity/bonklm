 ...
 * 13 cumulative-hardening). Centralises:
 *
 ...
 *
 * NOTE: per Story 2.4 hardening B2-rev, `modelName` is NOT a
...
export function isUnsafeBinaryResult(result: unknown): boolean {
  ...
  if (typeof globalThis.Buffer !== 'undefined' && globalThis.Buffer.isBuffer(result)) {
    return true;
  }
