...
expectNotAssignable<BonklmServerOptions>({ hmacSecret: 's', port: 0 }); // listen options belong to Fastify.listen
...
expectError(mapPortkey({}));
expectType<ExpectedMappedGuardInput>(
  mapPortkey({
    eventType: 'beforeRequestHook',
    request: { json: { messages: [{ content: 'x' }], model: 'm', prompt: 'p' }, text: 't' }
  })
);
expectType<ExpectedMappedGuardInput>(
  mapPortkey({ eventType: 'beforeRequestHook', messages: [{ content: 'x' }], model: 'm' })
);
