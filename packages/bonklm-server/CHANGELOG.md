...
- Validate instruction-bearing request fields consistently across supported server payloads and
  sanitize validator-provided response metadata at the HTTP boundary.
- Require an explicit trusted TLS-termination assertion before the production CLI binds a
  non-loopback host, while keeping loopback development available without the assertion.
- Updated dependencies
