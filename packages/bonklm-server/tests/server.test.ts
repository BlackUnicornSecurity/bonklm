 ...
 *
 * Acceptance criteria:
 *   1. Fastify service: `createBonklmGuardrailServer({ validators, hmacSecret })`.
...
describe('Story 2.13 — bonklm-server', () => {
  ...
  describe('Configuration validation', () => {
    ...
    it('measures the HMAC secret minimum in UTF-8 bytes', async () => {
      const server = await createBonklmGuardrailServer({
        validators: [new PromptInjectionValidator()],
        hmacSecret: 'é'.repeat(16)
      });
      await server.close();
    });
  ...
  // ── Story 2.13 hardening regressions ───────────────────────────
  ...
  describe('Audit BLOCK closures (Story 2.13 3-lane review)', () => {
    ...
    describe('sec S7 — confused-deputy merge across LiteLLM envelopes', () => {
      ...
      it('detects injection hidden in top-level Portkey messages when request.json is benign', async () => {
        ...
        expect(res.json()).toEqual({ verdict: false });
        await server.close();
      });
    });
    ...
    // v0.5.0 release review security review closure: makeDecision now
    ...
    describe('makeDecision sanitizes validator-controlled response strings', () => {
      ...
      class AttackerControlledValidator {
        ...
        async validate(): Promise<GuardrailResult> {
          return {
            ...
            findings: [
              {
                category: `attacker${ATTACKER_CONTROL_CHARS}_category`,
                severity: `blocked${ATTACKER_CONTROL_CHARS}` as Severity,
      ...
      it('strips control chars from reason + finding.description in dev mode (productionMode=false)', async () => {
        ...
        expect(decision.findings[0].category).not.toContain('\x00');
        expect(decision.findings[0].category).toContain('attacker');
        expect(decision.findings[0].severity).not.toContain('\x00');
        expect(decision.findings[0].severity).toContain('blocked');
        await server.close();
      });
    ...
    // v0.5.0 release review rev v5#6 closure: replaced `as never`
