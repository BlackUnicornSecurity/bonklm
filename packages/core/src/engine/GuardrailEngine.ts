 ...
 * ```
 *
 * @public v1.0-RC1 API freeze. Class + ALL public method
...
export class GuardrailEngine {
  /**
   * engine instance identifier. Random-per-construction so
  ...
  constructor(config: GuardrailEngineConfig = {}) {
    // assign per-engine instance ID first so any throw
    ...
    // empty-list fail-safe (spec-strict).
  ...
  /**
   * review BLOCK-3 — `validateInput` is the discriminated-union
...
/**
 * generate a per-engine random instance ID. Uses Web Crypto
