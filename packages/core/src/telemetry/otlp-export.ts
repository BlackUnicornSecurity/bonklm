/**
 * OTLP span export for validator decisions
 * ===================================================================
 *
 * `bonklmTrace(result, opts)` emits a single OTLP span per validator
 * decision with the locked attribute vocabulary:
 *
 ...
 * ```
 *
 * @public v1.0-RC1 API freeze. Caller-provides-tracer
 * contract is frozen — bonklm will never instantiate its own SDK /
 * exporter / processor. Surface vocabulary (locked) is frozen.
 */
export function bonklmTrace<R extends GuardrailResult>(result: R, options: BonklmTraceOptions): R {
  ...
  // wire format — same threat class as the finding-event
  // sites. Wrap at the boundary, defensive-by-default.
  ...
  options.tracer.startActiveSpan(spanName, { attributes }, span => {
    ...
    if (Array.isArray(result.findings) && span.addEvent) {
      for (const finding of result.findings) {
        // Retrofit the `sanitizeLogString(String(x ?? '<default>'))` combo to
        // the canonical helper `sanitizeMeta`. The `??
    ...
    if (action === 'block' && span.setStatus) {
      ...
      // Switched to `sanitizeMeta` for consistency
