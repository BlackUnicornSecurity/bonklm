...
export class ConsoleTelemetryCollector implements TelemetryCollector {
  ...
  collect(event: TelemetryEvent): void {
    ...
    // The langchain-handler fix sanitizes at the warn-log
  ...
  flush(): void {
    ...
    for (const event of events) {
      try {
        ...
        // sweep which missed this nested-class site).
  ...
  record(event: TelemetryEvent): void {
    ...
    for (const collector of this.collectors) {
      try {
        ...
        // CWE-117 sweep closure: a raw `{ error }` renders `error={}` post-JSON.stringify because
