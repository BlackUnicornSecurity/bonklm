...
import { detectIndirectInjection, redactIndirectInjection } from './indirect-injection-patterns.js';
...
export class IndirectInjectionValidator implements Validator {
  ...
  /**
   * {@link RedactingValidator} capability — used by `applyRedaction` in
   * RetrievedDocValidator `onPerDocFailure: 'redact'` mode. It re-runs the
   * surface-gated arms against the ORIGINAL `content` and substitutes each
   * matched region, rather than relying on `applyRedaction`'s substring
   * fallback over `Finding.match`: that fallback cannot locate an arm's region
   * because `detectIndirectInjection` redacts credential material out of `match`
   * ({@link redactSecrets}), so the pre-redacted string no longer appears
   * verbatim in the document. Only the configured (composition-path) surface is
   * redacted; with no surface the provenance gate is closed and content is
   * returned unchanged.
   */
  redactContent(content: string, replacement: string = '[REDACTED]'): string {
    ...
    return redactIndirectInjection(content, surface, replacement);
  }
}
