 ...
 *     once.
 *   - The contract also requires bidirectional detection ("payload split
 ...
 *   - soft cap 32KB (warn-only telemetry), hard cap 200KB
 ...
 *     the recall context.
 *   - telemetry attributes `composedContextBytesScanned`,
 *     `composedContextTruncated`, `composedContextSoftCapExceeded` are
 *     populated on `result.metadata` so the OTel adapter can
 ...
 * if (r.result.blocked) {
 * // contract: terminate the LLM invocation
...
export function createComposedContextValidator(config: ComposedContextValidatorConfig): ComposedContextValidator {
  ...
  const validateEntries = async (entries: string[]): Promise<ComposedContextBatchResult> => {
    ...
    // scan reverse-ordered concat too so attacks split across
