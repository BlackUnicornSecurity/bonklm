 ...
 *
 * **Surface vocab**: result.metadata.surface = 'text_input'.
 *
...
const PYTHON_PATTERNS: CodeInjectionPattern[] = [
  {
    name: 'python_eval',
    // hardening security BLOCK-2: allow optional block comment between
...
const JS_PATTERNS: CodeInjectionPattern[] = [
  ...
  },
  {
    ...
    // hardening code-reviewer CONCERN-4: `const {unlinkSync} = await
...
const SHELL_PATTERNS: CodeInjectionPattern[] = [
  ...
  },
  {
    ...
    // hardening code-reviewer CONCERN-3: drop the leading-whitespace
  ...
  },
  {
    ...
    // hardening architect BLOCK-1 + code-reviewer BLOCK-2 (jQuery FP):
  ...
  },
  {
    ...
    // hardening architect CONCERN-4: backtick fires on benign Markdown
...
const NETWORK_EGRESS_PATTERNS_BASE: Array<{ name: string; tool: string; pattern: RegExp; description: string }> = [
  {
    name: 'curl_egress',
    tool: 'curl',
    // hardening code-reviewer CONCERN-2: capture either a dotted host
...
const PACKAGE_INSTALL_PATTERNS: CodeInjectionPattern[] = [
  {
    name: 'pip_install',
    // hardening security CONCERN-1: `pip3.11 install` evades `pip3?`.
...
/**
 * @public v1.0-RC1 API freeze. `name = 'code_injection'`
 * is frozen. The sandbox-attack-corpus graduation gate (100/0/100)
 * anchors the detection contract.
 */
export class CodeInjectionValidator implements Validator {
  ...
  private isAllowlisted(host: string): boolean {
    // hardening security BLOCK-1 + architect CONCERN-1: suffix-exact
