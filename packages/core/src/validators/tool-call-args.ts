 ...
 *
 * Round-2 amendment: connectors that support runtime tool
...
export function createToolCallArgsValidator(config: ToolCallArgsValidatorConfig): Validator {
  ...
  // the tool_result surface onto the caller's chain. Ordering + surface tag live
  // in the shared composer (appended → caller validators run/short-circuit
  // first); adds tool-result task-hijack / exfil coverage no user-text bar carries.
  const validators = appendIndirectInjectionArm(config.validators, 'tool_result');
  ...
  return {
    ...
    async validate(input: string | ValidatorInput): Promise<GuardrailResult> {
      ...
      // custom serializer is supplied — the contract requires the name to flow
