...
// =============================================================================
// UNICODE NORMALIZATION AND MANIPULATION DETECTION
// =============================================================================
...
/**
 * Remove line breaks that sit BETWEEN two non-whitespace characters
 * (`prev\nious` → `previous`). Legitimate prose breaks lines between
 * words (whitespace surrounds the break) or at a hyphen; a break
 * inside a bare word is the classic mid-word split evasion used to
 * defeat keyword patterns. Newlines between words are preserved so
 * line-oriented consumers (secret scanning, line numbers) stay intact
 * — apply this only on the detection copy inside the prompt-injection
 * and jailbreak pipelines. Whitespace OTHER than line breaks (tab,
 * U+2028, U+3000, …) is folded to plain spaces by `normalizeText`
 * BEFORE this pass, so those mid-word variants are instead covered by
 * the concatenated-copy scan (`detectPatternsConcatenated`) —
 * collapsing all `\s` here would destroy inter-word phrase matching.
 */
export function collapseIntraWordLineBreaks(text: string): string {
