...
/**
 * @public v1.0-RC1 API freeze. `name = 'path_traversal'`
  ...
  private async validateString(content: string): Promise<GuardrailResult> {
    ...
    if (this.checkSymlinks && findings.length === 0) {
      try {
        ...
        // hardening code-reviewer BLOCK-1: use sep-bounded inside
        ...
        // hardening security CONCERN-3: fail-SECURE on realpath
