/**
 * AudioStreamValidator (CORE)
 * ========================================
 * Voice / realtime transcript validator. Wires `BufferedReleaseGate`
 * (primitive) as the partial-buffer; runs a curated
 ...
 * 'audio_partial'` (locked vocabulary). Partial-result
 * `metadata.surface` and final-result `metadata.surface` are stamped
 * with this constant so the OTel adapter can emit
...
export interface AudioStreamPartialResult {
  ...
  /** surface vocab — always 'audio_partial'. */
  surface: HookSurface;
}
...
/**
 * @public v1.0-RC1 API freeze. `name = 'audio_stream'`
 * is frozen. Partial-transcript semantics + BufferedReleaseGate
