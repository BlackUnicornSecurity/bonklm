/**
 * `validateWithTimeoutSecure` shared primitive.
 *
 * Background: an audit discovered that
 * fastify-plugin's timeout used a broken `AbortController`
 ...
 *
 * The fix was ported to fastify-plugin + nestjs-module. The
 * sweep found the SAME broken pattern in 20+ other
 ...
 * ```
 *
 * SECURITY-CRITICAL: this helper is the canonical timeout
 ...
 * Splunk, etc.).
 *
 * Moved the canonical implementation to
...
export interface ValidateWithTimeoutOptions<R> {
   ...
   *
   * Must be a positive finite number. Values ≤ 0, NaN, Infinity,
   * or non-numbers cause an
   ...
   *     rejection and crash the process on Node ≥15). Surfaced at
   *     WARN (not DEBUG) so operators see
...
/**
 * Minimum shape every `R` must satisfy. The helper's
 ...
 * that matters for the security boundary — on timeout, the
...
/**
 * race a validation operation against a timeout, returning
 ...
 *
 * @public v1.0-RC2 stabilization. The helper signature is
...
export async function validateWithTimeoutSecure<R extends TimeoutSentinelShape>(
  ...
  // memoize sentinel construction so
