...
vi.mock('../connectors/registry.js', async importOriginal => {
  ...
  // A connector whose only env var is optional — its prompt accepts an empty
  // answer and the skipped value is never persisted.
  const optomaConnector: ConnectorDefinition = {
    id: 'optoma',
    name: 'Optoma',
    category: 'utility',
    npmPackage: '@blackunicorn/bonklm-optoma',
    detection: { envVars: ['OPTOMA_HOST'] },
  ...
  return {
    ...
    getConnectorIds: () => [...actual.getConnectorIds(), 'mocktor', 'optoma', 'ghostless']
  };
});
...
describe('connector add action — optional credentials', () => {
  beforeEach(() => {
    primeActionMocks();
    h.testConnectorWithTimeout.mockResolvedValue({ connection: true, validation: true, latency: 3 });
  });

  it('offers a skippable prompt and persists nothing when left blank', async () => {
    h.password.mockResolvedValue('');

    const cmd = await loadConnectorAdd();
    await cmd.parseAsync(['optoma'], { from: 'user' });

    const prompt = h.password.mock.calls[0][0];
    expect(prompt.message).toContain('optional');
    expect(prompt.validate('')).toBeUndefined();
    // `connector add` always writes (EnvManager merges), but the skipped value
    // must not appear in the payload.
    expect(h.envWrite).toHaveBeenCalledWith({});
  });

  it('persists an optional credential the user does provide', async () => {
    h.password.mockResolvedValue('http://optoma.internal:4242');
  ...
  it('still rejects an empty value for a required credential', async () => {
    h.password.mockResolvedValue('mocktor-secret-value');

    const cmd = await loadConnectorAdd();
    await cmd.parseAsync(['mocktor'], { from: 'user' });

    const validate = h.password.mock.calls[0][0].validate;
    expect(h.password.mock.calls[0][0].message).not.toContain('optional');
