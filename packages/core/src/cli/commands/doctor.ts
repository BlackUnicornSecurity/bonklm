 ...
 *
 * The doctor ships with a single check — pre-commit hook installation
...
import { getConnectorsByCategory } from '../connectors/registry.js';
 ...
 * a request-flood attack against the validator pipeline
 * (10K req/s → validator pipeline).
 *
 * Derived from the connector registry rather than hardcoded — a hardcoded copy
 * silently went stale every time a framework connector was added, which is the
 * drift class the registry exists to remove. The `framework` category is not
 * identical to "has an HTTP ingress surface", so the few members that terminate
 * no requests are excluded by name rather than by category.
 *
 * @returns The npm package names of every `framework`-category connector.
 * @internal
 */
export const NON_INGRESS_FRAMEWORK_CONNECTORS = new Set([
  // A callback handler around an LLM client, not an HTTP surface — a rate
  // limiter advisory would be a false positive.
  '@blackunicorn/bonklm-langchain'
]);

function getFrameworkConnectorPackages(): string[] {
  return getConnectorsByCategory('framework')
    .map(connector => connector.npmPackage)
    .filter((name): name is string => name !== undefined && !NON_INGRESS_FRAMEWORK_CONNECTORS.has(name));
}
 ...
 * time. See `docs/user/security/rate-limiting.md` for the
...
export function checkRateLimiterAdvisory(cwd: string): DoctorCheckResult {
  ...
  const installedConnectors = getFrameworkConnectorPackages().filter(name =>
    Object.prototype.hasOwnProperty.call(allDeps, name)
  );
 ...
 *
 * B.14: validates that `cwd` exists and is a directory before
...
export function runDoctor(
  ...
  // before. Now we short-circuit with a clear failure entry.
 ...
 *
 * ### Sanitization contract (B.16 — ADR-0001 alignment)
 *
