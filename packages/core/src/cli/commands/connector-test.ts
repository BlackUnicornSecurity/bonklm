...
import { isOptionalEnvVar } from '../connectors/credential-format.js';
...
export interface ConnectorTestDeps {
  ...
  timeout?: number;
  /**
   * Registry lookup (defaults to {@link getConnector}). Injectable so a test can
   * exercise a connector shape the shipped registry does not contain yet — e.g.
   * one mixing a required and an optional credential.
   */
  getConnectorFn?: (id: string) => ConnectorDefinition | undefined;
}
...
export async function runConnectorTest(id: string, deps: ConnectorTestDeps = {}): Promise<ConnectorTestReport> {
  ...
  const connector = getConnectorFn(id);
  if (!connector) {
    return { status: 'unknown-connector', connectorId: id, exitCode: 1 };
  }

  const config = await loadConfig(connector);
  // Optional env vars (settings with a working default, or alternative provider
  // secrets) are excluded: their absence is not "unconfigured" and must not
  // block the test. See ConnectorDefinition.optionalEnvVars.
  const envVars = (connector.detection.envVars ?? []).filter(envVar => !isOptionalEnvVar(connector, envVar));
  const missing = envVars.filter(envVar => !(envVar in config));

  // "Not configured" = the connector declares required credentials but none are
  // present. A partially-configured connector still runs the test; the
  // connector's own test() surfaces the specifics as a validation failure.
  // Compare against `missing`, not the whole config: `defaultLoadConfig` loads
  // optional env vars too, so a lone optional value would otherwise satisfy
  // this check and let a connector with no required credential run its test.
  if (envVars.length > 0 && missing.length === envVars.length) {
