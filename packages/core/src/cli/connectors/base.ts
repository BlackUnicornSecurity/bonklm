 ...
 * - framework: Framework / web-server integrations (Express, Fastify, NestJS, etc.)
 * - vector-db: Vector database services (Chroma, Weaviate, Qdrant, etc.)
 * - agent: Agent frameworks and orchestration runtimes (Mastra, ElizaOS, MCP, etc.)
 * - memory: Long-term memory / conversation stores (Mem0, Zep, Letta)
 * - sandbox: Remote code-execution sandboxes (E2B, Daytona)
 * - workflow: Durable job / workflow runtimes (Inngest, Temporal, Trigger.dev, Restate)
 * - observability: Tracing and telemetry adapters (VoltOps / OpenTelemetry)
 * - utility: Shared helper packages with no external service to configure
 *
 * The list is additive: categories exist to group the connector list in the
 * wizard's selection prompt and in `bonklm status`. Widening it does not change
 * any connector's behaviour.
 */
...
/**
 * All {@link ConnectorCategory} values, in display order.
 *
 * Single source of truth for {@link isConnectorCategory} and for any consumer
 * that needs to enumerate categories without a live registry.
 */
...
export interface ConnectorDefinition {
  ...
  /**
   * The npm package that ships this connector (e.g.
   * `@blackunicorn/bonkdrant`).
   *
   * Set on every first-party connector. It is what makes the registry
   * auditable against the workspace: `registry.test.ts` asserts that every
   * publishable `packages/*` manifest is claimed by exactly one connector, so a
   * newly added connector package fails the build until it is registered. It is
   * also the package the wizard tells the user to install.
   */
  npmPackage?: string;
  ...
  /**
   * Env vars from {@link DetectionRules.envVars} that the user may leave blank
   * at the interactive prompt.
   *
   * The CLI prompts for every declared env var and rejects an empty value by
   * default, which is right for a connector that cannot work without its API
   * key. It is wrong for two real cases: a setting with a working default (the
   * Ollama host) and a connector that supports several alternative providers
   * where only one secret applies (the voice webhooks). Listing an env var here
   * makes its prompt skippable; a skipped value is not written to `.env`.
   *
   * @example
   * ```ts
   * detection: { envVars: ['OLLAMA_HOST'], ports: [11434] },
