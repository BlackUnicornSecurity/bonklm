...
// Export EdgeHookManager (edge-core — function-only, edge-safe).
export {
  EdgeHookManager,
...
/**
 * default surface when callers omit
...
export class HookManager<TContext extends HookContext = HookContext> {
  ...
  // one-shot deprecation warning when `surface` omitted.
  ...
  registerHook(definition: Omit<HookDefinition<TContext>, 'id'>): string {
    ...
    // cross-subsystem CWE-117 sweep: `hook.name` is
    ...
    for (const hook of hooks) {
      ...
      try {
        ...
        if (result.shouldBlock) {
          // cross-subsystem CWE-117 sweep: same `hook.name`
        ...
        // cross-subsystem CWE-117 sweep: switch raw
        ...
        results.push({
          success: false,
          shouldBlock: false,
          // `hook.name` + `error` flow into `HookResult.message`
          // returned to caller. Sanitize at the construction site so
          // the value is safe everywhere downstream (matches
