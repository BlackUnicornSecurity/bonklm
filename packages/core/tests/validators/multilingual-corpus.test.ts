...
describe('Multilingual Corpus — recall + FPR baseline (Sprint 16 measurement; gate from Sprint 22)', () => {
  ...
  /**
   * Sprint 17 hardening (code-reviewer BLOCK-2 + security BLOCK-1/2):
   * per-language corpus-driven TP gate. Activates per-language as
   * patterns land. Sprint 17 enforces bn + ur (both ≥ 85% recall after
   * the hardening pattern fixes). Hindi defers until Sprint 21
   * pattern PR.
   */
  it('per-language TP recall gate: bn + ur enforced at 85% (Sprint 17 hardening)', async () => {
    const detector = new MultilingualDetector();
  ...
  it('per-language FPR ceiling: bn + ur ≤ 5% (Sprint 17 hardening)', async () => {
    const detector = new MultilingualDetector();
    const ENFORCED_LANGS = new Set(['bn', 'ur']);
    for (const lang of languages) {
      if (!ENFORCED_LANGS.has(lang)) continue;
