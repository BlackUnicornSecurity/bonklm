...
describe('PromptInjectionValidator', () => {
  ...
  describe('ReDoS guard (B.1 / ST-05-101)', () => {
     ...
     * closing "-->" — this forced the old /<!--([\s\S]*?)-->/g regex into
    ...
    it('bounds worst-case input under 100ms (ReDoS guard B.1) — html_comment unclosed openers', () => {
      ...
      const validator = new PromptInjectionValidator({ detectHtmlComments: true });

      const t0 = performance.now();
      const result = validator.validate(worstCase);
      const elapsed = performance.now() - t0;

      expect(elapsed).toBeLessThan(100);
      // No injection detected — unclosed openers produce no findings
      expect(result.blocked).toBe(false);
    });
    ...
    it('still detects injection keyword in a closed HTML comment (B.1 correctness)', () => {
      ...
      expect(result.blocked).toBe(true);
      const htmlFinding = result.findings?.find((f: { category: string }) => f.category === 'html_comment_injection');
      expect(htmlFinding).toBeDefined();
    });

    /**
     * Generic 10 KB worst-case through the full validate() path.
     * This covers the MAX_INPUT_LENGTH guard (100k ceiling) plus all
    ...
    it('bounds worst-case input under 100ms (ReDoS guard B.1) — ml_javascript_escape near-miss', () => {
      ...
      const validator = new PromptInjectionValidator({ detectMultiLayerEncoding: true });
