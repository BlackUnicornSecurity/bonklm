...
describe('isHighEntropy', () => {
  it('should use default threshold of 3.5', () => {
    // NOTE: common sequences (alphabet runs, keyboard rows) are
    // rejected by the sequence blacklist BEFORE entropy scoring —
    // 'abcdefghijklmnopqrstuvwxyz0123456789' is NOT a secret no
    // matter its score. Use a shuffled high-entropy fixture.
    const highEntropy = 'v7Kq2LmX9Rp4ZnZ8Qw3Jt6Hs1Yb5Cd0Ef';
    expect(isHighEntropy(highEntropy)).toBe(true);
    expect(isHighEntropy('aaaa')).toBe(false);
    expect(isHighEntropy('abcdefghijklmnopqrstuvwxyz0123456789')).toBe(false);
  });
...
// ---------------------------------------------------------------------------
// sanitizeLogString zero-width / Unicode-format hex-escape (security regression)
...
// ---------------------------------------------------------------------------
// sanitizeLogString astral TAG-block + residual-Cf hex-escape (security regression)
// The BMP format pass above stops at U+FEFF and deliberately skipped the astral
// Unicode TAG block (U+E0000..U+E007F) -- the modern "ASCII smuggling" channel
// where a full readable ASCII payload is encoded as invisible tag chars an LLM
// processes but humans/grep never see -- plus residual Cf/deprecated-format
// points the detection-layer normalizer strips. The TAG block is astral, so the
// 4-hex \uNNNN formula mis-escapes (lone surrogate); the source uses a
// code-point-aware pass emitting the braced \u{NNNNN} form for astral points.
// Raw chars come from String.fromCodePoint (no embedded invisible bytes); the
// expected escape is hardcoded so a bug in the source formula cannot self-mask.
// ---------------------------------------------------------------------------
describe('sanitizeLogString -- astral TAG-block + residual-Cf regression corpus', () => {
  const ASTRAL_AND_RESIDUAL: ReadonlyArray<readonly [number, string]> = [
    // residual BMP Cf / deprecated-format (4-hex form)
    [0x00ad, '\\u00ad'], // SOFT HYPHEN
    [0x115f, '\\u115f'], // HANGUL CHOSEONG FILLER
    [0x1160, '\\u1160'], // HANGUL JUNGSEONG FILLER
    [0x180e, '\\u180e'], // MONGOLIAN VOWEL SEPARATOR
    [0x206a, '\\u206a'], // INHIBIT SYMMETRIC SWAPPING (deprecated format, range start)
    [0x206f, '\\u206f'], // NOMINAL DIGIT SHAPES (deprecated format, range end)
    [0xfff9, '\\ufff9'], // INTERLINEAR ANNOTATION ANCHOR
    [0xfffb, '\\ufffb'], // INTERLINEAR ANNOTATION TERMINATOR
    // astral TAG block (braced code-point form)
    [0xe0000, '\\u{e0000}'], // TAG block start
    [0xe0041, '\\u{e0041}'], // TAG LATIN CAPITAL A (an ASCII-smuggling carrier)
    [0xe007f, '\\u{e007f}'] // CANCEL TAG (block end)
  ];

  it('escapes every astral TAG-block + residual-Cf point and preserves surrounding text', () => {
    for (const [cp, escaped] of ASTRAL_AND_RESIDUAL) {
      const raw = String.fromCodePoint(cp);
      const result = sanitizeLogString(`before${raw}after`);
      expect(result).not.toContain(raw); // invisible char neutralised
      expect(result).toContain(escaped); // forensic signal preserved
      expect(result).toContain('before');
      expect(result).toContain('after');
    }
  ...
  it('neutralises an ASCII-smuggling payload encoded in the astral TAG block', () => {
    // Attack: an invisible instruction is encoded char-by-char as U+E0000+ascii,
    // rendering as nothing (humans + a naive grep miss it) while an LLM or a
    // Unicode-aware renderer reads the hidden ASCII. The log sanitizer must make
    // the smuggle visible (escaped) rather than let it ride invisibly into a SIEM.
    const smuggle = Array.from('ignore', c => String.fromCodePoint(0xe0000 + c.charCodeAt(0))).join('');
    const result = sanitizeLogString(`doc body ${smuggle} <-- hidden`);
    expect(result).not.toContain(smuggle); // raw invisible payload neutralised
    expect(result).toContain('\\u{e0069}'); // 'i' (0x69) -> U+E0069, escaped
    expect(result).toContain('\\u{e006e}'); // 'n' (0x6e) -> U+E006E, escaped
    expect(result).toContain('doc body ');
  });

  it('leaves legitimate astral chars (emoji) and BMP text intact -- no false positives', () => {
    // False-positive guard: the astral pass must touch ONLY the TAG block +
    // the named residuals, never ordinary astral glyphs (emoji / supplementary
    // CJK). U+1F680 ROCKET sits just outside the escaped ranges.
    const input = `deploy ${String.fromCodePoint(0x1f680)} ${String.fromCodePoint(0x20bb7)} ok`;
    const result = sanitizeLogString(input);
    expect(result).toBe(input);
  });
});
