 ...
 *
 * Story 2.3 hardenings apply here transitively:
...
// Note: review-rev-B4 removed an unused ALREADY_VALIDATED_SENTINEL
...
export function wrapEko<T extends EkoLike>(client: T, engine: GuardrailEngine, options: WrapEkoOptions = {}): T {
  ...
  assertNonCuaMode('wrapEko', client, {
    allowCuaMode,
  ...
  (client as { run: T['run'] }).run = validatedRun;
  (guarded as unknown as { run: T['run'] }).run = validatedRun;
  ...
  if (client.agents !== undefined && typeof client.agents === 'object' && client.agents !== null) {
    ...
    for (const [name, raw] of Object.entries(client.agents)) {
      ...
      if (typeof agent.act === 'function') {
        wrapBrowserAgentInPlace(agent, guarded);
      }
      ...
      if (typeof agent.read === 'function' || typeof agent.write === 'function' || typeof agent.delete === 'function') {
        wrapFileAgentInPlace(agent, guarded);
      }
    }
  }
  ...
  if (client.mcp !== undefined && typeof client.mcp.callTool === 'function') {
    wrapMcpInPlace(client.mcp, guarded);
  }
...
export function wrapEkoBrowserAgent<A extends EkoBrowserAgentLike>(
  ...
  wrapBrowserAgentInPlace(agent, guardedStub);
  return agent;
}
...
export function wrapEkoFileAgent<A extends EkoFileAgentLike>(
  ...
  wrapFileAgentInPlace(agent, guardedStub);
  return agent;
}
...
function wrapBrowserAgentInPlace(
  ...
  if (typeof agent.extract === 'function') {
    ...
    agent.extract = async <U = unknown>(
      ...
      let result: U;
      try {
        result = await originalExtract(opts as never);
      } catch (sdkErr) {
