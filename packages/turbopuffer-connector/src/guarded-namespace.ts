 ...
 * connector-style guide (`write` / `query` / `deleteAll`) and passes every other
 ...
 */
// arch X3 closure (review): value imports route through the
 ...
 *
 * arch X7 closure (review): `multiQuery` is added because
...
function makeWriteWrapper(
  ...
  return async function write(params?: GuardedNamespaceWriteParams | null, options?: unknown): Promise<unknown> {
    if (params === null || typeof params !== 'object') {
      // rev R0 closure (review): SDK signature is
...
/**
 * arch X7 closure (review): `multiQuery` returns
