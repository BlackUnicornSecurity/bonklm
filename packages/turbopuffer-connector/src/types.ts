 ...
 *
 * The connector-style guide mandates `createGuardedNamespace(ns, opts)` —
 * shape #2b (Vector-database sub-client wrap with validators-in-opts)
 * per the connector-style-guide. Mirrors the
...
export interface GuardedNamespaceOptions {
   ...
   *
   * @security sec S-TPUF-7 (review): the connector holds a
   ...
   * @default 'text'
   *
   * @security sec S1 closure (review, inherited): consumers
   ...
   *
   * @security sec S2 closure (review, adapted): silent
   ...
   * @default 1000
   *
   * @security sec S6 closure (review, inherited).
   */
   ...
   * @default 'block'
   *
   * @security rev R2 closure (review, inherited).
   */
 ...
 * Story 2.11 AC methods + `multiQuery` added at hardening per
...
export interface GuardedNamespace {
   ...
   * arch X7 closure (review): the connector wraps
