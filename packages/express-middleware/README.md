...
| `unparsableBodyPolicy` | `'block' \| 'scan-literal'` | `'block'`                   | Unparsable request bodies (circular refs, BigInt) are **blocked** by default (fail-closed). `'scan-literal'` restores the legacy behavior of scanning a placeholder literal (fail-open — not recommended). |

## Security Features

- **Path Traversal Protection**: Uses `path.normalize()` to prevent path traversal attacks
- **Request Size Limits**: Blocks requests exceeding configured size limit
- **Validation Timeout**: Prevents DoS via slow validations
- **Production Mode**: Generic error messages in production to prevent info leakage

## Security Best Practices

### Rate Limiting

This middleware does **not** include rate limiting. We recommend adding a rate limiter **before**
the guardrails middleware to prevent brute force and DoS attacks:
...
For production deployments, add security headers using
...
2. **Input Validation**: Use a validation library like [joi](https://www.npmjs.com/package/joi) or
