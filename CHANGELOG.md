...
Sixteenth patch of the 1.0.x line — publish lane completes end to end.

### Fixed
- The release bundle normalizer now reads GNU long-name (`@LongLink`) tar entries in addition to
  PAX overrides, so the lane's pack-and-verify steps agree on the GNU-tar CI image.
- Staging container cleanup no longer fails a completed publish: the registry refuses manifest
  deletions for the Actions identity (GHCR answers them UNSUPPORTED), which is a token-scope
  limitation, so the refusal is now warned on and left to retention, matching the tag-delete path.
...
[Unreleased]: https://github.com/BlackUnicornSecurity/bonklm/compare/v1.0.16...HEAD
